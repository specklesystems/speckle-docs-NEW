...
### Options and settings - **Embed code** (iframe) - **Embed URL** Use the
direct viewer URL if you need a quick link to open the model in a new browser
tab without embedding code. - **Options and Preview** Customize and toggle the
following: Transparent background, Hide viewer controls, Hide the selection info
panel, Prevent scrolling (zooming), Load model manually.
