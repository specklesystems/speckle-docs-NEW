...
<Note>
  ProjectWise updates are detected by scheduled checks. The integration is read-only and focuses on model and file synchronization rather than general document management.
</Note>
