...
When you publish a Revit model, all **3D perspective views** are automatically published as cameras — no extra steps needed.

<Info>
  Only 3D perspective views are published. Plan/section/elevation views and orthographic 3D views are not included.
</Info>

Published views are accessible under **Camera Controls** in the Speckle web viewer, and can be loaded in other supported host apps (e.g., Rhino).

When you load a model that contains views/cameras in Revit, they are automatically created as **3D views** in Revit. Views published from other connectors (e.g., Rhino named views) are also received. 

<AccordionGroup>
  <Accordion title="What happens if a 3D view with the same name already exists in Revit?">
    The existing view is skipped and not overwritten. Rename the view in the source app and reload to bring it in under a new name.
  </Accordion>
</AccordionGroup>