---
title: Getting Started
description: "Step-by-step guide for using the Power BI Connector"
---
<Note>Power BI connector is load-only.</Note>

Welcome! This guide will walk you through how to **load** and **visualize** a model inside Power BI using **Power BI connector**.

Speckle's connection to Power BI consists of two parts.
- Power BI Connector lets you easily load your model from Speckle into Power BI in a tabular format.
- 3D Viewer Visual allows you to view and color your models in 3d.

Here's how you can quickly get started with Power BI connector:

<Note> Before you begin, make sure you have a Speckle account and [installed Power BI Connector](http://localhost:3000/connectors/installation).</Note>

<Steps>
    <Step title="Open Power BI connector">
    1. Select **Get Data**.
    2. In the search box, enter *Speckle*.
    3. Select **Connect to Speckle** and click **Connect**.
    </Step>

    <Step title="Load from Speckle">
    1. Copy Model URL from the web app.
    2. Paste it in the text field and select **OK**.
    </Step>

    <Step title="Import 3D Visual">
    1. In the Visualizations pane, select the three dots **(...),** then select **Import a visual from a file**.
    2. Browse to `Documents/Power BI Desktop/Custom Visuals`.
    3. Select `Speckle 3D Visual.pbiviz` and **Open**.
    </Step>

    <Step title="Visualize a model">
    1. In the **Visualizations** pane, select **Speckle 3D Visual**
    2. Configure the visual:
        - Drag *Version Object ID* column into the **Version Object ID** input -> required for **viewing**
        - Drag *Object IDs* column into the **Object IDs** input ->  required for **interactivity**
        - Drag any column into **Tooltip** input -> for **tooltip**
        - Drag any column into **Color by** input -> for **coloring** model
    3. Wait for your model to load in the viewer
    </Step>
</Steps>

<Tip> Check the video and steps in the [first steps](/get-started/quickstart.mdx) section to get started even faster!</Tip>

**Congratulations! 🥳**  
You've just learned how to load a model in Power BI using Speckle! Now you can make your models accessible in seconds—enhancing collaboration and boosting productivity across your team.

<Info> If you have any questions or run into issues, visit our [community space](https://speckle.community/). We’re here to help!</Info>

import LearnMore from "/snippets/learn-more.mdx";

<LearnMore />
