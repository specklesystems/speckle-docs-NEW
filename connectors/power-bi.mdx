...
When you load a model, the Speckle connector automatically adds a **properties** column to your table with all the object properties.

Want to turn those properties into individual columns? Use the `Speckle.Utils.ExpandRecord` helper function.

**Example scenario:** You've loaded a model with room elements where the **properties** column contains room data like names, numbers, areas, and other details.

**Expand all properties:**
```powerquery
Speckle.Utils.ExpandRecord(Source, "properties")
```
- `Source` = your model table
- `"properties"` = the column you want to expand

Now each property gets its own column, making them easy to use in your Power BI reports.

**Extract only what you need:**

Keep things clean by extracting specific properties. To get just room names, numbers, and areas from your rooms table:

```powerquery
Speckle.Utils.ExpandRecord(Source, "properties", {"Name", "Number", "Area"})
```

This pulls out only the properties you specify, creating three new columns: **Name**, **Number**, and **Area**.

### Extracting Material Quantities

### Extracting Composite Structure
