...
If you are working with Rhino model objects, you can easily create Speckle Properties from the model object user text via casting [link to casting section]. To create or modify custom Speckle Properties, there are two options depending on the complexity and size of your property sets.
...
1. Add a default **Property** input of the value of that property (see supported types above).
    ...
    Right-click and select **List access** if the value of the input is a list instead of a single item.
...
If your team has a standard for property sets on objects, you can create a [reusable node](/grasshopper-best-practices.mdx) with your Create Properties nodes that define a specific property set for your project model.
...
1. Add a **Properties** input to see all concatenated keys of all nested properties inside that properties set.
    Use the **search bar with regex search** [link to regex] to filter the displayed keys if needed.
2. Click to select any keys, or shift + click to select multiple keys

```
// add a "?" in front of your search query to only display items that contain your query
...
<Tip>
If you frequently access the same property keys across multiple objects or models, you can create a [reusable node](/grasshopper-best-practices.mdx) with your Property Selector and Query Properties nodes to retrieve specific properties.
</Tip>
