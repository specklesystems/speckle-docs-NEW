...
<AccordionGroup>
    ...
    <Accordion title="What is the difference between the Survey Point and Shared Coordinates reference points?">
      Both reference points are based on Revit's survey point, but they differ in rotation:

      - **Survey Point** — positions the model at the survey point with no rotation applied. True North is ignored, so the model is oriented the same way as Revit's internal coordinate system relative to the survey point.
      - **Shared Coordinates** — positions the model at the survey point **and** applies the True North rotation. This matches the full shared coordinate system in Revit, where the model is rotated to reflect the project's real-world orientation.
