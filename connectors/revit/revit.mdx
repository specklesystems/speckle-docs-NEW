...
next_docs:
  - /next/connectors/revit/introduction
change_scope: 'Revit placement and Apply Transform'
---
...
Affected guidance: Revit placement and Apply Transform.

For these changes, read [2026.9 guidance](/next/connectors/revit/introduction).
...
<Note>
  Update the Revit connector from **1 September 2026**, and no later than **1 November 2026**.
  Existing connectors keep working. For Apply Transform, the placement picker, and how reference
  points work in 2026.9, see [Revit in 2026.9](/next/connectors/revit/introduction).
</Note>
...
<Accordion title="What is the difference between the Survey Point and Shared Coordinates reference points?">
  Both are based on Revit's survey point, but differ in rotation: - **Survey Point** positions the
  model at the survey point with no rotation. - **Shared Coordinates** positions the model at the
  survey point and applies the True North rotation. In short: **Shared Coordinates = Survey Point +
  True North rotation**.
</Accordion>
...
<AccordionGroup>
    ...
    <Accordion title="Why don't my custom properties or parameters appear on loaded objects in Revit?">
      Loading properties onto Revit parameters is not supported at this time. When you load a model, the Revit connector creates the geometry, name, category, and render material of each object, but it does not write any of the object's properties (for example Rhino user text, Grasshopper attributes, or parameters published from another Revit model) into Revit parameters.

      This is the case even if you have created shared parameters in Revit with matching names, or structured the property keys to match the `Parameters` layout you see on published Revit objects. Publishing a new version with changed values will not populate the parameters either.

      The properties are still on your Speckle model and can be viewed in the web viewer or read through the [API](/developers/introduction). Populating Revit parameters from loaded data is a tracked feature request; in the meantime you can write the values with a Revit-side script such as Dynamo or pyRevit.
    </Accordion>
