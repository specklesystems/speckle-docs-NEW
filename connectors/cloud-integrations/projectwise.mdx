...
<AccordionGroup>
  ...
  <Accordion title="How quickly are file changes synced?">
    Speckle checks ProjectWise on a schedule, so a change may take a few minutes to appear. When a change is detected, Speckle creates a new model version from the file version current at that check. Versions superseded between checks, or whose sync fails, are not synced later; see [Sync guarantees](/it-admin/integrations/introduction#sync-guarantees).
  </Accordion>
