...
When you publish a Rhino model, all **named views** are automatically included — no extra steps needed.

**Note:** Only perspective named views are published. Parallel projection views are not included.

When you load a model that contains views, they are automatically created as **named views** in Rhino.

- If a named view with the same name already exists in Rhino, it will be skipped and not overwritten.
- Views published from other connectors (e.g., Revit 3D perspective views) are also received as named views in Rhino.

Published views are accessible under **Camera Controls** in the Speckle web viewer, and can be loaded in other supported host apps (e.g., Revit).
