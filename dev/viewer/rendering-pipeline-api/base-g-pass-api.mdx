...
[GPass](/dev/viewer/rendering-pipeline-api/g-pass-api) -> [BaseGPass](/dev/viewer/rendering-pipeline-api/base-g-pass-api)

When developing a new pass, it's recommended to extend this class rather then implementing the `GPass` interface directly.


<Warning>
  `BaseGPAss` implements almosts all [GPass](/dev/viewer/rendering-pipeline-api/g-pass-api) members however the `render` function is declared as `abstract` so extending implementations will need to implement it themselves
</Warning>
...
- **layer**: The [ObjectLayers](/dev/viewer/viewer-api#objectlayers) to enable/disable
