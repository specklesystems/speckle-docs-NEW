...
<AccordionGroup>
  ...
  <Accordion title="How quickly does a new version show up in Speckle?">
    ...
    time it takes to convert the file. Speckle syncs the file version current at each check;
    versions superseded between checks, or whose sync fails, are not synced later. See [Sync
    guarantees](/it-admin/integrations/introduction#sync-guarantees).
  </Accordion>
