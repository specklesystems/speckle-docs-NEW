...
<Warning>
  Known issue: geometry can disappear or flicker at certain zoom levels, most often reported for
  objects far from the model origin or in models with very large bounds. This is a rendering-stability
  issue in the viewer itself, not specific to Power BI — the same viewer now runs in the web app,
  the Power BI visual, and published reports. This release prioritized streaming performance and
  large-model capability; the fix lands in the web viewer first, then ships in the following Power
  BI visual release, which is not on the same cadence.
</Warning>
