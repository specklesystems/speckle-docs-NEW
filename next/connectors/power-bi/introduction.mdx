...
<Warning>
  Firefox on macOS currently allows eight GPU storage buffer bindings, and the 3D visual needs nine,
  so it will not render in Power BI Service reports opened there. Use a Chromium-based browser or
  Power BI Desktop instead.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="What is missing from the 2026.9 3D visual?">
    Section box, view modes, edges, ghost mode, and named scene views are unavailable. Filtered-out
  ...
  <Accordion title="Which browsers and environments does the 3D visual support?">
    Power BI Desktop's embedded WebView2 and Chromium-based browsers viewing Power BI Service both
    support the visual. Firefox on macOS does not: it supports WebGPU, but currently allows only
    eight GPU storage buffer bindings, and the visual needs nine. Other browsers on macOS allow ten
    or more. Windows and Android browsers generally allow sixteen or more. This is a known, accepted
    limitation. Firefox may raise the limit later.
  </Accordion>
  <Accordion title="Is ghost mode available in the 2026.9 3D visual?">
    Not yet. Filtered-out objects are hidden outright instead of shown dimmed. There is no ghost
    icon or equivalent toggle in this release.
  </Accordion>
  <Accordion title="Why does geometry disappear or flicker when I zoom in?">
    This is a known rendering-stability issue in the viewer, not specific to Power BI — the same
    viewer now runs in the web app, the Power BI visual, and published reports. It shows up most
    often with objects far from the model origin or in models with very large bounds. The fix ships
    in the web viewer first, then in the following Power BI visual release, which is not on the same
    cadence.
  </Accordion>
</AccordionGroup>
