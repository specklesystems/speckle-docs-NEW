...
v2026.9 replaces that reconstruction with an **explicit relational layer**: typed, directed edges
between objects (and, where needed, structural nodes such as a level, material, or grouping
container). For the two-store picture, see [Object model in v2026.9](/next/developers/object-model/overview).
...
encode connectivity, hosting, or room occupancy as a graph a consumer can query. v2026.9 keeps the
...
Names below are the live vocabulary of the relational layer (the same names the v2026.9 viewer
...
Material and color are separate on purpose. An object can carry both. Geometry-level material is
the intrinsic appearance; object-plane color is presentational and can override geometry color.
...
<Note>
  `DISPLAY_INSTANCE` is the render placement. Do not treat every object-to-instance association as a
  render root. Definition members that must stay linked without drawing at the origin use a separate
  association (`PLACES`) on producers that emit it.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Can I keep walking Collection.elements for hierarchy?">
    That walk is the Current tree. On v2026.9 data, use `IN_COLLECTION`, `IN_MODEL`, `ON_LEVEL`, and
    `SUBELEMENT` instead of assuming a nested `elements` graph. See [Object model in
    v2026.9](/next/developers/object-model/overview).
  </Accordion>
</AccordionGroup>
