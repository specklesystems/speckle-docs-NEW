...
description: Typed containment, connectivity, geometry, systems, and materials — richer than today's Proxies.
contextual:
  options: []
docs_version: '2026.9'
...
Before 2026.9, as the [Data Schema](/developers/data-schema/overview) describes, overlapping membership is a
...
Proxies themselves do not exist in 2026.9 bundle data - there is no `RenderMaterial`, `Level`,
`Group`, `Definition`, `InstanceProxy`, or `InstanceDefinitionProxy` object anywhere in the format.
The relation types below are what carries that information instead.
...
{/* IMAGE_PLACEHOLDER: 2026.9 viewer inspector — named relations on a selected object (ON_LEVEL, HOSTED_ON, CONNECTS_TO). */}
...
| Relation        | From → to                            | Replaces, in 2026.8 and earlier                                    | When you use it                                                                                         |
| --------------- | ------------------------------------ | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
...
These are new as a queryable graph. Before 2026.9 there is no Proxy equivalent.
...
An object's own geometry is a relation too, not a `displayValue` array on the object. The blob
format behind these rows is on [Geometry encoding in
2026.9](/next/developers/object-model/geometry-encoding).

| Relation     | From → to         | Replaces, in 2026.8 and earlier                                     | When you use it                                                                                                            |
| ------------ | ----------------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `DISPLAY`    | object → geometry | `displayValue` meshes on the object                                 | The object's own render mesh (walls, in-place elements). Instanced objects use `DISPLAY_INSTANCE` instead                  |
| `SOLID`      | object → geometry | The host solid (for example a Rhino Brep) beside its `displayValue` | A true solid body where the host has one (Rhino, Civil 3D). Reach for it when you need the body, not the tessellated shell |
| `CENTERLINE` | object → geometry | `baseCurve` / `baseLine` on the object                              | The axis a duct, pipe, conduit, or framing member follows. Routing, length take-off, clash lines, and single-line drawings |

`DISPLAY` is what renders. `SOLID` and `CENTERLINE` are additional shapes for consumers that need
the body or the axis. Do not draw every object-to-geometry relation, or the centerline appears
through the middle of the duct.

### Materials and appearance

| Relation              | From → to            | Replaces, in 2026.8 and earlier                       |
| --------------------- | -------------------- | ----------------------------------------------------- |
...
The Definition and Instance Proxies of 2026.8 and earlier become structural nodes plus relations.
...
<Steps>
  ...
  <Step title="Identify objects by applicationId">
    ...
    outcome is a stable object set across versions. If you are producing data rather than only
    reading it, this only holds if every placement's `applicationId` is unique — a repeated id
    resolves to the same object, not a second occurrence.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Can I keep walking Collection.elements for hierarchy?">
    That walk is the tree of 2026.8 and earlier. On 2026.9 data, use `IN_COLLECTION`, `IN_MODEL`, `ON_LEVEL`, and
    `SUBELEMENT` instead of assuming a nested `elements` graph. See [Object model in
    2026.9](/next/developers/object-model/overview).
  </Accordion>
  <Accordion title="Can I still share one object between parents with a detached reference?">
    No. Detaching and the `reference` object type belong to the object graph of 2026.8 and earlier;
    a bundle has no `__closure` to resolve them through. Publish the shared thing as its own object
    and relate the others to it, or, if what you are sharing is really a value rather than an
    object, write that value as a property on each object. See [Object model in
    2026.9](/next/developers/object-model/overview).
  </Accordion>
  <Accordion title="Is CENTERLINE always the geometric centre of the element?">
    No. It is the authored location curve. A Revit wall's location line follows its Location Line
    setting and can be a core or finish face. A point-placed MEP fitting (elbow, tee, cross) has no
    location curve, so it emits one `CENTERLINE` segment per connector, ordered by connector.
  </Accordion>
  <Accordion title="Can I express a solid modelling feature tree with relations?">
    No, and that is not the intent. Relations describe how finished objects relate: containment,
    hosting, connectivity, appearance, instancing. They do not describe how an object was made, and
    nothing in Speckle rebuilds geometry from a recipe. What you can carry: the exact body through
    `SOLID` in the host's own format, feature parameters as properties, and ownership through
    `SUBELEMENT`. A dependency order can travel in your own edge file, which no Speckle consumer
    reads. Exchanging modelling history would need operation node kinds in the catalog and a
    modelling kernel on the receiving side. Neither exists today.
  </Accordion>
  <Accordion title="Can I describe an assembly and its parts with relations?">
    Yes, as a resolved structure. Parts are objects. A sub-assembly owns its parts through
    `SUBELEMENT`, or joins them through `IN_ASSEMBLY` when membership is the point. A part used
    many times is one definition placed by instances, each with its own transform, and a repeated
    sub-assembly is a nested definition. Part-to-part contact is `CONNECTS_TO`, and exact bodies
    ride `SOLID`. What does not travel is the constraint system: mates such as coincident or
    concentric, degrees of freedom, and the solver that positions parts from them. You publish the
    positions those constraints produced, not the constraints.
  </Accordion>
  <Accordion title="Can I extend the relation topologies for my tools?">
    ...
    numbers for future platform types. Which cases need a custom file, and which are really an
    existing relation or a property, is on [Building integrations in
    2026.9](/next/developers/building-integrations#relations-you-do-not-need-and-relations-you-add).
  </Accordion>
  <Accordion title="What happens if two objects share the same applicationId?">
    They are treated as the same object referenced twice, not as two occurrences.
    `applicationId` is not required on every object, but wherever it is relied on as identity — a
    `DISPLAY_INSTANCE` or `DEFINES_INSTANCE` placement, a `Flatten().ToDictionary(o =>
    o.applicationId)` lookup, any join back to a source element — that identity has to be unique
    for the objects relying on it. A producer that reuses an id across two distinct placements
    will see the second one silently collapse onto the first: same node, same transform, no error
    raised.
  </Accordion>
</AccordionGroup>
