...
If you write scripts, notebooks, or small automations against Speckle object data, v2026.9 changes
...
Collection for shared resources. v2026.9 replaces that tree-and-Proxy graph with two things working
...
In v2026.9, object identity stays stable (`applicationId` from the source application). Attributes
...
v2026.9 puts those links beside the property store as **typed relations**. Containment, connectivity,
...
<Steps>
  ...
  <Step title="Follow typed relations for topology">
    ...
    v2026.9](/next/developers/object-model/relations).
  </Step>
</Steps>

<Warning>
  If you maintain tooling that reads Speckle data more directly than the standard SDK or API
  surface, and it breaks against the new format, pin to **v2026.8** while you update. Compatibility
...
<AccordionGroup>
  <Accordion title="Does the Current Data Schema still describe my data?">
    ...
    [Proxy schema](/developers/data-schema/proxy-schema) for that graph. Use this page for v2026.9
    bundle data.
  </Accordion>
  ...
  <Accordion title="What happens if my script walks Collection.elements and unpacks Proxies?">
    That reconstruction matches the Current model. On v2026.9 data, containment and shared resources
    ...
    property paths, and follow named relations — or pin to v2026.8 until you can.
  </Accordion>
  <Accordion title="Where do I learn the relation type names?">
    See [Relations in v2026.9](/next/developers/object-model/relations). That page maps today's
