...
description: Before-and-after Speckle.Sdk examples so you can judge 2026.9 migration impact.
contextual:
  options: []
docs_version: '2026.9'
...
<Tabs>
  ...
  <Tab title="2026.9 (Receive3)">
    ```csharp 2026.9 lines icon="/images/developers/sdks/csharp.svg"
...
`Send3` returns a reserved version id immediately. Queries, the version URL, and the
`version_create` webhook fire when ingestion completes. Poll for the version, or watch the
