...
Any version you are asked to load may still be a legacy object graph. Read the version record
first.

| Field                      | Legacy object graph            | 2026.9 bundle                              |
| -------------------------- | ------------------------------ | ------------------------------------------ |
...
Dispatch on the `bundle.` prefix. Keep your legacy reader for the first shape, or refuse it with a
...
<AccordionGroup>
  <Accordion title="What happens if the artifacts listing returns 404?">
    Either the version is a legacy object graph, the bundle has not been produced yet, or your token
    cannot read the project. Check `schemaVersion` on the version record first, then the ingestion
    status if the version is new.
  </Accordion>
