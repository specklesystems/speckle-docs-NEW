...
<Note>
  The bundle spec's source and its validator are not published today. What is public is the
  generated vocabulary, vendored into the open-source producers: `specklepy.bundle.spec` in Python,
  the `Libs/bundlespec` headers in the Archicad connector, and `artifacts/vocab.rb` in the SketchUp
  connector. Every bundle also carries its own `rel_types` and `node_kinds` catalogs, so a version
  published by any Speckle connector shows you the live vocabulary. Copy from the most recently
  updated of those and record which one you copied.
</Note>

<Note>
  Without the validator, your correctness loop is the server: publish to a test project, wait for
  the ingestion to reach `success`, open the version in the viewer, then receive it with an SDK and
  check that it reports no unknown relations and no geometry decode errors. Run that loop in CI.
</Note>
