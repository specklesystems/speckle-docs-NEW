...
<Warning>
  Running a function against a model in the 2026.9 data model works the same as always, but you
  currently can't see the results — not in the viewer, not on the project dashboard. See [Viewing
  Automate results in 2026.9](/next/developers/automate/viewing-results) before assuming a run
  failed. If your function attaches results to individual objects, see [Attaching results in
  2026.9](/next/developers/automate/attaching-results) too — that call has its own, unrelated
  caveat for 2026.9-format models.
</Warning>
...
<Note>
  An in-app **Open docs** control on this page may still point at an old URL. Use the Automate
  guides linked above, not that control.
</Note>
...
<AccordionGroup>
  <Accordion title="Why is there no Automations tab on the project?">
    ...
    [Create Automation](/developers/automate/create-automation) describe the previous UI.
  </Accordion>
