...
<Note>
  Reports and [Dashboards](/next/analytics/dashboards) are currently distinct products. Dashboards
  are available on all plans and are built manually; Reports are generated from an Intelligence Chat
  conversation and require Intelligence. How the two develop alongside each other is something we're
  actively working out — if you have a Dashboards workflow that shapes what you'd want from Reports,
  [we'd like to hear it](https://speckle.community).
</Note>
...
The toolbar also offers **Undo** and **Redo** for layout edits made in the editor. Undo does not
apply to AI turns — those changes land as a new edit state you can undo or discard.
...
{/* IMAGE_PLACEHOLDER: Report editor toolbar — Add menu open showing Chart KPI Table Viewer Text; Unsaved changes Save Discard Undo Redo; ⋯ More actions. */}

### Properties panel

Clicking any element in the editor opens the **Properties** panel on the right. What you see depends on what is selected.

**Section** — Set **Title**, **Show title**, **Subtitle**, and **Show subtitle**. Drag the section
using the grip in its menu. Convert a plain section to a tabbed section with **Add tabs**, or add
another tab to an existing tabbed section with **Add tab**.

**Row** — Set **Height** (in pixels) to control how tall the row renders. A row can contain
multiple cells side by side.

**Widget** — Set a **Title** (required) and optional **Description**. For charts, you can also
switch the **Chart type** between bar, line, and scatter — available on AI-generated charts only,
not hand-authored ones. Chart **Labels** lets you rename what the chart shows (axis labels, series
names) without affecting the underlying values; leave a field empty to keep the original.

{/* IMAGE_PLACEHOLDER: Properties panel — three states: section (title subtitle show-title), row (height field), widget (title description chart-type labels). */}

Use Chat on the report page to ask the AI to create or rewrite content — those turns spend AI
credit. Direct edits in the Properties panel do not.
