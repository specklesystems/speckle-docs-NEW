...
description: What changes for Data Validation after models move to the 2026.9 data format, including related objects and value mapping.
noindex: true
contextual:
  options: []
---
...
{/* IMAGE_PLACEHOLDER: Check editor — Add related object; Host element / Room / Connected element; path picker on the related object. */}
...
<Note>
  **Add related object** is disabled when the preview version has no relationship data. Publish a
  new version with a current connector after the project uses the 2026.9 data format. Related-object
  rules then show that there is no data, rather than failing the whole run.
</Note>

## Map labels before you compare

{/* IMAGE_PLACEHOLDER: Equals property comparison control — Compare equals / at least / at most; This value maps / Other value maps; Add mapping. */}
...
{/* IMAGE_PLACEHOLDER: Path-matching control — match every property whose path ends with Fire Rating. */}
...
<AccordionGroup>
  ...
  <Accordion title="Why is Add related object disabled?">
    ...
    after the project uses the 2026.9 data format. See [Checks against related
