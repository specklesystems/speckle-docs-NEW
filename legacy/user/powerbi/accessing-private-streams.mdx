...
7. After selecting a server, follow **Edit Permissions > Edit > Private Model.** Paste your _Token_ into `Personal Access Token` input.

That's it! Now, Power BI will continue fetching your private data from the Speckle server.

<Tip>
  Delete existing servers if you have trouble seeing your server under Data sources. Return to the
  Speckle Connector and try to receive the same model/version. You'll have the option to add it from
  there.
</Tip>
