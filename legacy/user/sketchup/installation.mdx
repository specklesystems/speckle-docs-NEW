---
title: 'Installation'
noindex: true
product_generation: legacy
docs_status: superseded
docs_authority: historical
change_scope: Legacy connector guidance; the existing notice recommends newer connectors.
replacement_docs:
  - /connectors/overview
---

import LegacyWarning from '/snippets/legacy-warning.mdx'

<Visibility for="agents">
  Documentation generation: legacy. Status: superseded. Authority: historical.

Legacy connector guidance; the existing notice recommends newer connectors.

For newer connectors, start with [replacement documentation](/connectors/overview).

Use this page only for the legacy connector it describes. The existing notice says it may be out of date. For Speckle 2026.9 differences, see [Connectors in 2026.9](/next/connectors/overview). This is not evidence of product removal or a one-to-one replacement.

</Visibility>

<LegacyWarning connector="SketchUp" />

To install the SketchUp Connector, it's assumed you have:

- Speckle Account (you can get one **for free🎉** at **[https://app.speckle.systems/](https://app.speckle.systems/)**)
- Manager for Speckle. You can download it from **[here](https://speckle.systems/download/)** and add your Speckle account.

Once Manager is installed and your account is set, you can continue with the installation.

## Installing on Windows and Mac

<Frame>
  <video
    autoPlay
    muted
    loop
    src="/images/legacy/sketchup/installing-sketchup-connector.mp4"
    type="video/mp4"
    alt="Installing sketchup connector"
    aria-label="Installing sketchup connector"
  />
</Frame>

1. Open Manager for Speckle and click on the "**Connectors**" tab.
2. You should see a list of available connectors. Scroll down until you find the "**SketchUp Connector**" or use the search bar to find it.
3. Click the "**Install**" button next to it.

<Warning>
  If you are using a Mac with an Apple Silicon chip, you will need to install
  [Rosetta](https://support.apple.com/en-us/HT211861) before installing the SketchUp Connector.
</Warning>

That's it! You're all set to use the Speckle connector for SketchUp.

## Installing early releases

If you want to stay on the cutting edge of the SketchUp Connector, then you can install pre-releases of it.

<Frame>
  <video
    autoPlay
    muted
    loop
    src="/images/legacy/sketchup/pre-releases.mp4"
    alt="Pre releases"
    aria-label="Pre releases"
  />
</Frame>

To install pre-releases for the SketchUp Connector, follow these steps:

1. Inside the connector page, switch to the "**Versions**" tab.
2. Inside the "**Versions**" tab, toggle on the "**Show Pre-releases**" option.
3. From this list, select the desired version you wish to install and click on the "**Install**" button next to it.

<Warning>
  ⚠️ Please keep in mind that these early versions are not intended for production work and may have
  some untested features.
</Warning>
