...
Gets this render view's [_GeometryType_](/developers/viewer/render-view-api#geometrytype-enum).
...
<Warning>
  By default, `batchStart` and `batchCount` are dynamic, so _they can change_ at runtime.
  `vertStart` and `vertEnd` are not dynamic by default.
</Warning>
<Warning>
  _Normally_, you have no need overwritting the render view's batch data. It's handled internally by
  the viewer-core.
</Warning>
...
<Warning>
  The render view's aabb can be read by using [_aabb_](/developers/viewer/render-view-api#aabb), but
...
- **attributes**: [_GeometryAttributes_](/developers/viewer/render-view-api#geometryattributes) Virtual vertex attribute arrays. Contains an array of original data chunks that make up the geometry
