...
<Tip>
  Extensions typically need to support being turned on/off with no impact on potentially other
  active extensions.
</Tip>
**Returns**: boolean
...
- **view**: Explicit view of different possible type: [_CanonicalView_](/developers/viewer/extensions/camera-controller-api#canonicalview), [_SpeckleView_](/developers/viewer/viewer-api#speckleview), [_InlineView_](/developers/viewer/extensions/camera-controller-api#inlineview)
- **transition**: Whether or not to make the transition animated
- _optional_ **fit**: Linear tolerance
...
```typescript
type CanonicalView =
  'front' | 'back' | 'up' | 'top' | 'down' | 'bottom' | 'right' | 'left' | '3d' | '3D'
```
