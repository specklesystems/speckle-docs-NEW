...
<Tip>
  These passes will be used for rendering in scenarios where the pipeline needs to render, however
  it wants to keep its last progressive render result and just pass it through
</Tip>
...
<Tip>
  These passes will be used for rendering in static scenarios, like when the camera has stopped and
  the pipeline needs to accumulate progressive samples
</Tip>
