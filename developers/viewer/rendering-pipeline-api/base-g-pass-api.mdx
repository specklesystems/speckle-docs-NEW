...
<Warning>
  `BaseGPAss` implements almosts all [GPass](/developers/viewer/rendering-pipeline-api/g-pass-api)
  members however the `render` function is declared as `abstract` so extending implementations will
