...
<Note>
  The "G" stands for "Graphics" and it's only there to make it clear that it's a different type that
  three's `Pass`.
</Note>
...
Gets the material that will override _all_ materials. If the overriden material is defined, it will completely replace any existing materials on the batches and no other effects that apply materials on objects will be made visible in this pass's render result. If `null`, nothing gets overriden
...
- _optional_ **scene**: The scene to render
...
<Warning>
  Clearing will be executed on whatever the current `outputTarget` value is. If `outputTarget` is
  `null`, the backbuffer will be cleared!
</Warning>
