...
- **subtreeId**: The [_TreeNode_](/developers/viewer/world-tree-api#treenode) to add as a subtree
...
<Warning>
  For non instanced nodes, this function will return the identity transformation since speckle does
  not define a local space (yet) and all geometry is in world space. This function **will not** take
  any runtime user defined transformations into account.
</Warning>
...
**Returns**: A dictionary where each instance id holds a record of [_TreeNode_](/developers/viewer/world-tree-api#treenode) grouped by their instance unique id.
...
**Returns**: [_TreeNode[]_](/developers/viewer/world-tree-api#treenode)
...
<Warning>Purges render trees are no longer usable.</Warning>
