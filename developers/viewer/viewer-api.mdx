...
- **params**: [_ViewerParams_](#viewerparams)

**Returns**: [**_Viewer_**](/developers/viewer/viewer-api)
...
**Returns**: [_QueryResult_](/developers/viewer/queries-api#queryresult)
...
interface Utils {
  ...
  NDCToScreen(x: number, y: number, width?: number, height?: number): { x: number; y: number }
}
```
...
- **type**: _AssetType_
  <Warning>For correct asset caching use need to use unique asset ids!</Warning>
