...
The other area to watch, separate from ACC sync and drag-and-drop specifically: code that reaches
into the **open-source Viewer package**'s internals, or into the lower-level
**`@speckle/objectloader` / `@speckle/objectloader2`** packages it's built on — extending render
...
is you, ask for compatibility mode too. By 1 November 2026, either an alpha release for the 2026.9
format will be available, or that deadline moves for developers in this position — see [Viewer
developer notes in 2026.9](/next/developers/viewer/introduction) for the latest on which.
...
integration is on, [ask us](https://speckle.community/help) rather than requesting compatibility
mode speculatively.
...
<AccordionGroup>
  ...
  <Accordion title="I embed @speckle/viewer and use its selection/filtering APIs — do I need compatibility mode?">
    No, not on that basis alone. Driving `@speckle/viewer` through its supported extensions and APIs
