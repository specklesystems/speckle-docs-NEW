...
<Note>
  ...
  `client.active_user.can_create_personal_projects()` before calling this method.
</Note>
### create_in_workspace()
...
<Warning>
  This method only works on workspace-enabled servers (e.g., app.speckle.systems). Check workspace
  permissions using `workspace.permissions.canCreateProject` before calling.
</Warning>
### update()
...
<Warning>
  This operation is irreversible! All models, versions, and data in the project will be permanently
  deleted.
</Warning>
### update_role()
...
<Card
  ...
  icon="user"
  href="/developers/sdks/python/api-reference/resources/active-user"
>
