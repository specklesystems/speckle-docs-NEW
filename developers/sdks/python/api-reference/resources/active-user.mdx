...
<Note>
  Email and other sensitive fields cannot be updated via the API. Use the web interface to change
  your email address.
</Note>
### get_projects()
...
<Info>
  This method is useful when you need to check permissions before attempting operations, helping you
...
<Note>
  Use the Speckle web interface to accept or decline invitations. This method only retrieves pending
  invitations. See [PendingStreamCollaborator](#pendingstreamcollaborator) for property details.
</Note>
...
<Warning>
  This method only works on workspace-enabled servers (e.g., app.speckle.systems). On other servers,
...
<Warning>This method only works on workspace-enabled servers (e.g., app.speckle.systems).</Warning>
## Types
...
<ResponseField name="invitedBy" type="LimitedUser">
  User who sent the invitation. See
