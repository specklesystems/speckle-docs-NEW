...
<ResponseField name="version" type="Tuple">
  Version tuple in format `(major, minor, patch)` or `(major, minor, patch, tag, build)` for
...
<Warning>The token is only shown once when created. Store it securely immediately!</Warning>
...
<Note>You can only revoke tokens that belong to your authenticated account.</Note>
...
<ResponseField name="authStrategies" type="List[AuthStrategy]">
  Available authentication strategies - `id` (str) - Strategy ID - `name` (str) - Strategy name -
...
<ResponseField name="author" type="dict">
  App author information - `id` (str) - Author user ID - `name` (str) - Author name - `avatar` (str,
