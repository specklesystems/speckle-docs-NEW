...
<Warning>
  Only set `verify_certificate=False` for local development or testing. Always verify certificates
  in production!
</Warning>
...
<Note>
  This method requires that you've previously logged in via Speckle Manager or a Speckle Connector
  (Rhino, Revit, etc.).
</Note>
...
When the SDK doesn't provide a method for a specific GraphQL operation, you can execute raw GraphQL queries. `execute_query()` covers queries with no variables; for anything that takes variables, call `execute()` on the client's underlying `httpclient` instead.
...
<Note>
  `execute_query()` is deprecated in favour of `client.httpclient.execute()`, which is used in the
  examples below and is also the only way to pass variables.
</Note>

Execute a raw GraphQL query against the Speckle server. It does not accept variables.
...
# Custom GraphQL query, no variables
query = """
    query CustomQuery {
        projects(limit: 100) {
            items {
...
`execute_query()` takes a plain query string and has no way to bind variables to it. When an operation needs variables, wrap it with `gql()` and call `execute()` on `client.httpclient` (the [`gql`](https://gql.readthedocs.io/) client SpeckleClient is built on), passing `variable_values`:
...
result = client.httpclient.execute(query, variable_values={"projectId": "your-project-id"})
```

### When to Use Custom Queries

Use a custom query or mutation when you need to:
...
result = client.httpclient.execute(
    mutation,
    variable_values={"input": {"field": "value"}},
)
```
