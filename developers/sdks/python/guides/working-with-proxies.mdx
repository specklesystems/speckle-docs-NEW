...
After receiving a commit with `operations.receive()`, objects have a `displayValue` property containing visualization geometry. This can be:
...
commit = operations.receive("commit_id", transport)
...
objects_with_display = find_displayable_objects(commit)
...
app_id_index = build_applicationid_index(commit)
...
commit.instanceDefinitionProxies = [
    {
        "id": "door_def_123",
        "name": "Door Type A",
        "objects": [
...
```python /dev/null/extract-instances.py#L1-40
...
# Usage
definitions = extract_instance_definitions(commit, app_id_index)
...
```python /dev/null/resolve-instances.py#L1-50
from specklepy.objects.geometry import Mesh
...
```python /dev/null/apply-transform.py#L1-45
...
# Usage
instances = get_transformed_instances(commit, app_id_index)
...
```python /dev/null/complete-example.py#L1-80
from specklepy.api import operations
from specklepy.transports.server import ServerTransport
from specklepy.api.client import SpeckleClient
from specklepy.api.credentials import get_default_account
...
def analyze_display_geometry(stream_id, commit_id):
    ...
    for context in traversal.traverse(commit):
        obj = context.current
...
# Run analysis
analyze_display_geometry("your_stream_id", "your_commit_id")
```
...
Not all commits have instance definitions:

```python /dev/null/check-proxies.py#L1-10
# ✅ Always check first
if hasattr(commit, "instanceDefinitionProxies"):
    definitions = extract_instance_definitions(commit, app_id_index)
else:
    print("No instance definitions in this commit")
    definitions = {}
...
# Use it to debug
for proxy in commit.instanceDefinitionProxies:
...
<Accordion title="Why are there no instance definitions in my commit?">

**Problem:** `commit.instanceDefinitionProxies` doesn't exist or is empty.
...
if hasattr(commit, "instanceDefinitionProxies") and commit.instanceDefinitionProxies:
    print(f"Found {len(commit.instanceDefinitionProxies)} instance definitions")
    definitions = extract_instance_definitions(commit, app_id_index)
else:
