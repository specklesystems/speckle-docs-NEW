...
<Note>
  For a comprehensive explanation of proxies and why Speckle uses them, see [BIM Data Patterns:
  Pattern
  5](/developers/sdks/python/guides/bim-data-patterns#pattern-5-level-and-location-data-with-proxy-collections)
  and [Proxification](/developers/sdks/python/concepts/proxification).
</Note>
...
**A note on duplicates:** `index[obj.applicationId] = obj` overwrites silently if two objects share an `applicationId` - no exception, no warning, the earlier object just disappears from the index. `applicationId` is expected to be unique per object in the graph; if lookups are returning the wrong object or an instance is missing its own transform, check for a producer-side id collision before assuming a bug in this code.
