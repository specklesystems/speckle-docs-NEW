...
<Note>
**Prerequisites:** This guide builds on traversal techniques. If you're new to navigating Speckle object graphs, start with [Traversing Objects](/developers/sdks/python/guides/traversing-objects) to learn the foundational patterns used throughout this guide.
</Note>
...
## Pattern 3: DisplayValue Proxies
...
## Pattern 4: DataChunk References

Large arrays are automatically chunked for performance.

### Understanding DataChunks

```python lines icon="python"
# When you receive a mesh, chunks are automatically reassembled
mesh = operations.receive(mesh_id, remote_transport=transport)

# You access the full array as normal
print(f"Vertices: {len(mesh.vertices)}")  # All vertices available
print(f"Faces: {len(mesh.faces)}")  # All faces available

# Chunking happens transparently during send/receive
```

### Detecting Chunked Data

```python lines icon="python"
def analyze_object_size(obj):
    """Analyze object size and potential chunking."""
    info = {
        "type": obj.speckle_type,
        "chunked_properties": []
    }
    ...
    if hasattr(obj, "vertices"):
        info["vertices_count"] = len(obj.vertices)
        if len(obj.vertices) > 30000:
            info["chunked_properties"].append("vertices")
    
    if hasattr(obj, "faces"):
        info["faces_count"] = len(obj.faces)
        if len(obj.faces) > 60000:
            info["chunked_properties"].append("faces")
    
    return info

# Use it
mesh_info = analyze_object_size(large_mesh)
if mesh_info["chunked_properties"]:
    print(f"Likely chunked: {mesh_info['chunked_properties']}")
```

## Pattern 5: Instance and Definition

Complex models use instance/definition patterns for efficiency.

### Understanding Instances

```python lines icon="python"
# Instance pattern (like Revit families or Rhino blocks)
{
    "speckle_type": "Objects.Other.Instance",
    "definition": {...},  # The shared geometry/data
...
```python lines icon="python"
def find_instances_and_definitions(root):
    """Find all instances and their definitions."""
    instances = []
    definitions = {}
        ...
        if isinstance(obj, Base):
            ...
            if obj.speckle_type == "Objects.Other.Instance":
                ...
                if hasattr(obj, "definition") and obj.definition:
                    def_id = getattr(obj.definition, "id", None)
                    if def_id and def_id not in definitions:
                        definitions[def_id] = obj.definition
...
# Use it
instances, definitions = find_instances_and_definitions(building)
print(f"Found {len(instances)} instances")
print(f"Found {len(definitions)} unique definitions")

# Count instances per definition
from collections import Counter
def_types = [inst.definition.speckle_type for inst in instances if hasattr(inst, "definition")]
type_counts = Counter(def_types)
for def_type, count in type_counts.most_common():
    print(f"  {def_type}: {count} instances")
```

## Pattern 7: Level and Location Data with Proxy Collections
...
<CardGroup cols={2}>
  ...
  <Card title="Simple Data Patterns" icon="shapes" href="/guides/simple-data-patterns">
