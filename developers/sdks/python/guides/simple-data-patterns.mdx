...
<Warning>
  Properties can be any type (string, number, bool, list). Always use `.get()` with defaults for
  safety.
</Warning>
...
<Info>
  Always provide `displayValue` for custom objects so they're visible in Speckle viewers, even
...
<AccordionGroup>
  <Accordion title="Use SDK traversal utilities">
...
<Accordion title="Traversal: get_member_names() filters automatically">
  `get_member_names()` already filters out private members (starting with `_`) and methods:
  ```python # get_member_names() already excludes private members for name in
  obj.get_member_names(): value = getattr(obj, name, None) # No need to check startswith("_") # Only
  exclude specific properties you want to skip for name in obj.get_member_names(): if name !=
  "displayValue": # Skip specific property value = getattr(obj, name, None) ``` This makes traversal
