...
When you use `IOperations.Send` or `Receive` (Path B), you pass an `ITransport` that reads and writes serialized objects — to Speckle Server, a local SQLite cache, or in-memory storage for tests. If you use `Send2`/`Receive2` for server work, you usually do not construct transports yourself.
