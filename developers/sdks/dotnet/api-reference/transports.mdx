...
When you use **`IOperations.Send`** or **`Receive`** (Path B), you pass an **`ITransport`** that reads and writes serialized objects. If you use **`Send2`**/**`Receive2`**, you usually do not construct transports yourself.

**When to read this:** Path B workflows ([Quickstart](/developers/sdks/dotnet/getting-started/quickstart)). **Read next:** [Send and Receive Paths](/developers/sdks/dotnet/concepts/send-and-receive-paths) Path B vs C.
