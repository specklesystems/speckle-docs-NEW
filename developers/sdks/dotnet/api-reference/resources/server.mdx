...
Before you call workspace APIs on a self-hosted server, check what the connected server supports with **`client.Server`**.

**When to read this:** before `client.Workspace` or workspace-scoped `ActiveUser` methods. **Read next:** [Handling Server Capabilities and Permissions](/developers/sdks/dotnet/guides/handling-server-capabilities).
