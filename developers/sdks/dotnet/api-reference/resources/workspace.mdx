...
When your user works on a workspace-enabled server, use **`client.Workspace`** to read workspace details and list projects inside an organization.

**When to read this:** after `client.Server.IsWorkspaceEnabled()` returns true. **Read next:** [ProjectResource](/developers/sdks/dotnet/api-reference/resources/project) to create projects in a workspace.
