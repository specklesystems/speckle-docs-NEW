...
When a desktop connector uploads a large host-application model through `SendPipeline`, Speckle Server tracks that job as a **model ingestion**. Use `client.Ingestion` to create the job, observe status, and handle failures — most scripts and services never need this resource.
