...
When a GraphQL call fails, a transport cannot reach the server, or deserialization cannot resolve your types, Speckle.Sdk throws typed exceptions under `SpeckleException`. This page maps those types to common causes so you can handle failures in production code.

<Info>
For authentication-specific failures (invalid token, no local accounts), see [Authentication](/developers/sdks/dotnet/getting-started/authentication#common-authentication-errors).
</Info>
