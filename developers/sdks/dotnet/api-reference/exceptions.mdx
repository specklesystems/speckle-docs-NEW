...
When a GraphQL call fails, a transport cannot reach the server, or deserialization cannot resolve your types, Speckle.Sdk throws typed exceptions — most under `SpeckleException`, but GraphQL response errors specifically arrive wrapped in `AggregateException` (see [GraphQL Exceptions](#graphql-exceptions)). This page maps those types to common causes so you can handle failures in production code.
...
<CardGroup cols={2}>
  ...
  <Card title="Handling Server Capabilities and Permissions" icon="shield-check" href="/developers/sdks/dotnet/guides/handling-server-capabilities">
    Check first instead of catching broadly — the pattern Speckle's connectors use
  </Card>
</CardGroup>
