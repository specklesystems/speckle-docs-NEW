...
When a GraphQL call fails, a transport cannot reach the server, or deserialization cannot resolve your types, Speckle.Sdk throws typed exceptions.

**When to read this:** before production error handling. **Read next:** [Authentication](/developers/sdks/dotnet/getting-started/authentication#common-authentication-errors) for auth failures; [Handling Server Capabilities](/developers/sdks/dotnet/guides/handling-server-capabilities) for permission patterns.
