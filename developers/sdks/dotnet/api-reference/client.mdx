...
`IClient` is the authenticated entry point to Speckle Server's GraphQL API. It exposes every server-side resource (projects, models, versions, users, and more) as typed properties, and is built from an `Account` via `IClientFactory`.
...
<AccordionGroup>
  ...
  <Accordion title="How do I know if a server supports workspaces?">
    Call `client.Server.IsWorkspaceEnabled()`. Self-hosted servers may not have workspaces enabled, in which case `Workspace` resource calls will fail.
  </Accordion>
