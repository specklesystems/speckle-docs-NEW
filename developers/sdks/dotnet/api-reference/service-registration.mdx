...
description: "AddSpeckleSdk and the dependency injection surface"
---

Before you resolve **`IOperations`**, **`IClientFactory`**, or transport factories, call **`AddSpeckleSdk`** on your **`IServiceCollection`** once at startup.

**When to read this:** during project setup or when customizing registration. **Read next:** [Dependency Injection](/developers/sdks/dotnet/concepts/dependency-injection) for concepts; [Scripts and Notebooks](/developers/sdks/dotnet/getting-started/scripts-and-notebooks) for copy-paste bootstrap.
...
<AccordionGroup>
  <Accordion title="What is the minimum bootstrap for a script?">
    `new ServiceCollection().AddSpeckleSdk(new Application(...), "1.0.0").BuildServiceProvider()` — see the full helper in [Scripts and Notebooks](/developers/sdks/dotnet/getting-started/scripts-and-notebooks).
  </Accordion>
