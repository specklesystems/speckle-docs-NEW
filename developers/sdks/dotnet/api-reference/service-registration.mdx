...
`AddSpeckleSdk` is an extension method on `IServiceCollection` that registers every service Speckle.Sdk needs. See [Dependency Injection](/developers/sdks/dotnet/concepts/dependency-injection) for the conceptual explanation.
