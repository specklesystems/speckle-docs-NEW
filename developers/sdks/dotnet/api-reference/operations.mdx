...
```csharp
var results = await operations.Send2(client.ServerUrl, project.id, account.token, myData, null, cancellationToken);
var received = await operations.Receive2(client.ServerUrl, project.id, results.RootId, account.token, null, cancellationToken);
```
...
var pipeline = provider.GetRequiredService<ISendPipelineFactory>()
    .CreateInstance(ingestion, account, uploadProgress, cancellationToken);
