...
`IOperations` (resolved via DI) is the entry point for moving `Base` object graphs to and from storage. See [Send and Receive Paths](/developers/sdks/dotnet/concepts/send-and-receive-paths) for guidance on which method to use.
