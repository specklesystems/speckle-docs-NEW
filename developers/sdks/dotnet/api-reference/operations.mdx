...
After `WaitForUpload`, the server processes the uploaded pack and creates the version asynchronously — the caller does not call `client.Version.Create` on this pathway. See [Building Connector-Scale Sends](/developers/sdks/dotnet/guides/building-connector-scale-sends).
...
<AccordionGroup>
  ...
  <Accordion title="Does Send create a version automatically?">
    No for **`Send`** and **`Send2`** — sending registers objects on the server; you must call `client.Version.Create` with the returned `objectId` to publish data to a model. **`SendPipeline`** is different: after `WaitForUpload`, the server creates the version asynchronously — do not call `Version.Create`. See [VersionResource](/developers/sdks/dotnet/api-reference/resources/version) and [Building Connector-Scale Sends](/developers/sdks/dotnet/guides/building-connector-scale-sends).
  </Accordion>
