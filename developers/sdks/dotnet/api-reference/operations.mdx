...
When you need to move `Base` object graphs — serialize to JSON, send to Speckle Server, receive by id, or run connector-scale ingestion — you call methods on **`IOperations`** (or **`ISendPipelineFactory`** for Path E).

**When to read this:** after [Send and Receive Paths](/developers/sdks/dotnet/concepts/send-and-receive-paths) tells you which path fits. **Read next:** [VersionResource](/developers/sdks/dotnet/api-reference/resources/version) to publish a version after `Send`/`Send2`.
...
<AccordionGroup>
  ...
  <Accordion title="Does Send create a version automatically?">
    No for **`Send`** and **`Send2`** — call `client.Version.Create` after send. **`SendPipeline`** creates the version on the server after upload — see [Building Connector-Scale Sends](/developers/sdks/dotnet/guides/building-connector-scale-sends) and [VersionResource](/developers/sdks/dotnet/api-reference/resources/version).
  </Accordion>
