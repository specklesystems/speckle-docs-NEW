...
Before your code can create projects, send data, or call Speckle Server, Speckle.Sdk needs an **Account** (a token plus server and user metadata) and an **IClient** built from it.

**What this page covers:** the three .NET ways to obtain an Account — personal access token, interactive browser login, and locally stored accounts. **When to read it:** before any server call in a new app. **Read next:** [Scripts and Notebooks](/developers/sdks/dotnet/getting-started/scripts-and-notebooks) (token-based scripts) or [Quickstart](/developers/sdks/dotnet/getting-started/quickstart) (full workflow tour).
