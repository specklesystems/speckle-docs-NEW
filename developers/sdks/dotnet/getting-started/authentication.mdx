...
Speckle.Sdk authenticates by building an `Account` — a token plus server and user information — and passing it to `IClientFactory` to create an `IClient`. There are three ways to get an `Account`, depending on your application.
