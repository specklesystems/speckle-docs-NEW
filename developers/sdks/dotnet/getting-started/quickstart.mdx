...
1. Register Speckle.Sdk with dependency injection
...
Register Speckle.Sdk with dependency injection once, at startup. Pass the `Speckle.Objects` assembly so the geometry types used below can be deserialized later.
...
<Tip>
You don't need to understand dependency injection to follow this guide — copy this block once, then resolve services from `provider` as shown below. The [Dependency Injection](/developers/sdks/dotnet/concepts/dependency-injection) concept page explains why this exists.
</Tip>
