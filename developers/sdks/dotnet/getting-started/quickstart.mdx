...
<AccordionGroup>
  ...
  <Accordion title="Why doesn't my geometry show in the 3D viewer?">
    Geometry primitives sent at the root without a container object are not visible in the viewer. Wrap them in a `Base` object, as in Step 4. See [Display Values](/developers/sdks/dotnet/concepts/display-values) for more detail.
  </Accordion>
