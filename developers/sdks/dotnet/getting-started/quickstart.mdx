...
When you want to see how project creation, sending, versioning, and receiving fit together, this page walks the complete workflow step by step using **Path B** (`Send`/`Receive` with `ServerTransport`).

**When to use it:** first full tour of the SDK — not the shortest script path ([Scripts and Notebooks](/developers/sdks/dotnet/getting-started/scripts-and-notebooks) uses `Send2`). **Read next:** [Authentication](/developers/sdks/dotnet/getting-started/authentication) before Step 2.
