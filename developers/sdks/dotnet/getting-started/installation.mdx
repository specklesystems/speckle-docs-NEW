...
Speckle.Sdk is built around `Microsoft.Extensions.DependencyInjection`. Every real workflow — authenticating, sending, receiving — resolves its services from an `IServiceProvider`. Register the SDK once, at application startup:
...
<Note>
This is the only dependency injection setup you need to follow the [Quickstart](/developers/sdks/dotnet/getting-started/quickstart). The [Dependency Injection](/developers/sdks/dotnet/concepts/dependency-injection) concept page explains what `AddSpeckleSdk` registers and why, once you've completed your first send and receive.
</Note>
