---
title: "Enrich Speckle data in Grasshopper"
description: "Use Speckle.Sdk in a C# Script to build DataObjects and Collections from Grasshopper geometry and computed metadata, then inspect the object graph locally"
---

<Note>
Think of the Grasshopper connector as the **transport layer** and the SDK as the **data modelling layer**. The connector excels at moving data between applications; the SDK excels at constructing, enriching, and analysing the object graphs that are moved.
</Note>

This guide teaches you to build sophisticated Speckle object graphs inside Grasshopper — **computed metadata, nested collections, and graph-wide inspection** — without publishing to a server. When you are ready to exchange data, use [Publish](/connectors/grasshopper/grasshopper) on the connector or [Appendix A](#a-publish-the-same-graph-from-code) below.
...
Passthrough nodes move geometry and attach **static** attributes. They do not run graph-wide computation or build nested structures from rules. Reach for Speckle.Sdk in a C# Script when you need:

- **Derived metadata** per element — distance from origin, quadrant, utilization, nearest grid line — computed from geometry, not typed on sliders
- **Many elements** with consistent rules applied in one pass
- **Nested collections** grouped by computed fields
- **Graph-wide inspection** — count, summarize, or serialize the whole model locally before exchange
...
**Why wouldn't I just use the node?** Passthrough nodes excel at transport and light edits; they do not replace code when metadata must be **derived** from geometry or when structure follows programmatic rules. See [How this relates to the Speckle Grasshopper connector](#how-this-relates-to-the-speckle-grasshopper-connector) for the full comparison.
...
### Step 6a — Geometry on `displayValue`

Each Rhino `Point3d` becomes a Speckle `Point` on the `DataObject.displayValue` list. The viewer and connector both expect renderable geometry here — same field [Deconstruct](/connectors/grasshopper/grasshopper-developer) reads on a published param.

### Step 6b — Compute properties on `DataObject`

BIM-like fields live in `DataObject.properties`, not on the root `Base` indexer. The script derives every field from point position, grid proximity, and `capacity` — that is the justification for code over passthrough nodes.

### Step 6c — `Collection` hierarchy

Footings group into child collections by computed `quadrant`. Nested `Collection.elements` mirrors how [Create Collection](/connectors/grasshopper/grasshopper-collections) nodes organize a canvas — except the grouping rule runs in C#.

### Step 6d — Inspect locally

`root.Flatten()` counts every `Base` in the graph. `summary` reports aggregated computed fields. Optionally preview JSON:
...
<AccordionGroup>
  <Accordion title="Optional — Serialize for JSON preview">
    ...
    ```
    Use this to sanity-check property names and nesting before exchange. See [Send and Receive Paths](/developers/sdks/dotnet/concepts/send-and-receive-paths#path-a-serialize-only).
  </Accordion>
</AccordionGroup>
...
**Why wouldn't I just use the node?** Passthrough nodes move and lightly edit data on wires. They do not execute graph-wide computation, derive per-element metadata from geometry, or assemble nested collections from grouping rules. **Deconstruct** inspects one param at a time; **Speckle.Sdk** constructs and analyses the whole graph; **Publish** exchanges it.
...
- **Compiled component** with typed Speckle outputs — study [GrasshopperShared](https://github.com/specklesystems/speckle-sharp-connectors/tree/main/Connectors/Rhino/Speckle.Connectors.GrasshopperShared)
- **Automation off-canvas** — [Quickstart](/developers/sdks/dotnet/getting-started/quickstart)
- **After Load** — query patterns in [Finding and Extracting Data](/developers/sdks/dotnet/guides/finding-and-extracting-data)
...
<AccordionGroup>
  <Accordion title="A. Publish the same graph from code" id="a-publish-the-same-graph-from-code">
    If you already built `root` in the script and need to send it without the Publish component, add account bootstrap and follow [Scripts and Notebooks](/developers/sdks/dotnet/getting-started/scripts-and-notebooks):
    ...
    See [Send and Receive Paths](/developers/sdks/dotnet/concepts/send-and-receive-paths) and [Authentication](/developers/sdks/dotnet/getting-started/authentication) (Appendix C).
  </Accordion>
  <Accordion title="B. Fetch and query after Load">
    After **Load** on the connector, traverse received graphs with [Finding and Extracting Data](/developers/sdks/dotnet/guides/finding-and-extracting-data). Server access requires [Authentication](/developers/sdks/dotnet/getting-started/authentication).
  </Accordion>
