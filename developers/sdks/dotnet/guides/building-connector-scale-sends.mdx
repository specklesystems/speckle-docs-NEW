...
When a desktop connector converts an entire host-application model — tens of thousands of Revit, Rhino, or ArchiCAD elements — `Send` and `Send2` cannot hold the full graph in memory. `SendPipeline` streams objects to a local NDJSON pack, uploads once, and tracks progress through `client.Ingestion`.
...
<AccordionGroup>
  ...
  <Accordion title="Processing an entire object graph in one Process() call">
    `Process` only serializes the single object you pass it; it does not recurse into detached children. Passing your whole root object without first processing its children means those children are never written to the pack, and deserialization on receive will fail to resolve their references.
  </Accordion>
  ...
  <Accordion title="404 or unsupported errors calling client.Ingestion methods">
    The Model Ingestion API requires server version 3.0.3 or later. On 3.0.11+, use `model.CanCreateModelIngestion` to verify availability before enabling this pathway; on 3.0.3–3.0.10, confirm compatibility with your server admin.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Handling Server Capabilities and Permissions" icon="shield-check" href="/developers/sdks/dotnet/guides/handling-server-capabilities">
    Detect old-server incompatibility without brute-force try/catch
  </Card>
