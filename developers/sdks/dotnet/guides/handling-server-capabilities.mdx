...
- `client.Project.GetPermissions(projectId)` → `ProjectPermissionChecks` (`canCreateModel`, `canDelete`, `canLoad`, `canPublish`)
- `client.Model.GetPermissions(projectId, modelId)` → `ModelPermissionChecks` (`canUpdate`, `canDelete`, `canCreateVersion`)
- `client.ActiveUser.GetProjectsWithPermissions()` → each `ProjectWithPermissions.permissions` inline
