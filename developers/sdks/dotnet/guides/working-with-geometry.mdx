...
When your .NET code creates or reads 3D data — meshes for `displayValue`, points for analysis, or geometry after receive — you use types from `Speckle.Objects`. Storage rules and the full primitive catalog: [Geometry Schema](/developers/data-schema/geometry-schema).
...
<AccordionGroup>
  ...
  <Accordion title="Sent geometry doesn't appear in the viewer">
    Geometry must be in a container's `displayValue` — see [Display values](/developers/sdks/dotnet/concepts/objects-and-traversal#display-values). A bare `Mesh` at the root won't render.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Geometry Schema" icon="sitemap" href="/developers/data-schema/geometry-schema">
    Platform geometry storage rules and primitive types
  </Card>
  <Card title="Finding and Extracting Data" icon="magnifying-glass" href="/developers/sdks/dotnet/guides/finding-and-extracting-data">
    Traversal and filtering patterns
  </Card>
</CardGroup>
