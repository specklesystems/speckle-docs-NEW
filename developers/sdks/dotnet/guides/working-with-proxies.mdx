...
When you receive connector-produced data and need objects grouped by level, material, or instance — or geometry hidden inside instance definitions — you resolve **proxies** at the root of the package against an `applicationId` index. For the platform model (Definition/Instance, proxy structure, `objects` field semantics), see [Proxy Schemas](/developers/data-schema/proxy-schema).
...
`instance.transform` is a `Matrix4x4` — apply it to place each instance correctly.
