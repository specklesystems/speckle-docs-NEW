...
Property casing on custom types is your choice — Speckle serialises names as declared. The `Room` example above uses camelCase; `Wall` uses PascalCase. Both work. The `displayValue` key is different: it must be exact camelCase for the viewer regardless of how you name other properties.
