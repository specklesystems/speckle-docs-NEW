...
You don't construct references by hand. The serializer emits them when you **detach** a property, and it records the referenced `id` in the parent's `__closure` at the same time — which is what lets the object be found again on receive. Assign the same object to a detached property on several parents and it is stored once and referenced from each:
...
A reference is a pointer, and the viewer's selection panel shows it as one. Select an object whose property holds a reference and that property renders as the stub itself — `referencedId`, `speckle_type`, `closure` — not the referenced object's properties inlined. To read those, select the referenced object, which appears in the model tree in its own right.

If you want a value to be legible on each object in the viewer, set it on each object. References deduplicate storage; they don't propagate properties for display.
