...
```python specklepy
shared = Base()
shared.name = "SharedBracket"
shared.material = "steel"

# "@" marks the property detached; add_detachable_attrs({"sharedComponent"})
# does the same for a property you don't want to prefix.
child_1["@sharedComponent"] = shared
child_2["@sharedComponent"] = shared

operations.send(base=root, transports=[server_transport])
```

```csharp .NET
var shared = new Base { ["name"] = "SharedBracket", ["material"] = "steel" };

// "@" marks the property detached, as in specklepy.
child1["@sharedComponent"] = shared;
child2["@sharedComponent"] = shared;
```

</CodeGroup>

`SharedBracket` is stored once, both children carry a reference to it, and `operations.receive` resolves each one back to the full object.
