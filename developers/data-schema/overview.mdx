...
<Accordion title="Objects and Addressing">
Objects are scoped to projects—when an object is sent to a Speckle server, it's stored within a project's scope and receives a unique object ID (hash) based on its content. However, without being referenced by a version, the object is effectively **unreachable** through normal workflows—these are called "orphaned" or "zombie" objects.
...
- **Flat object graph**: Objects don't nest within other objects—they're organized in collections and referenced via proxies
...
2. DataObjects contain geometry in `displayValue` (not nested DataObjects)
...
This tree structure makes traversal predictable and efficient—you can walk from root to leaf without encountering cycles or complex reference resolution.
