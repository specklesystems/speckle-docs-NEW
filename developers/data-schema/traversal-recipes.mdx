...
Now that you understand the data model, here are practical patterns for traversing and processing Speckle models. These recipes assume you have a Root Collection object. To get one from a version, see [Versions and the root object](/developers/data-schema/version-root-object).
...
def find_objects_using_resource(root, proxy_type, resource_name):
    ...
    referenced_ids = proxy.objects if hasattr(proxy, 'objects') else []
...
def find_resources_for_object(root, obj):
    ...
    for proxy in get_all_proxies(root):
        if hasattr(proxy, 'objects') and app_id in proxy.objects:
...
- **[Connector Index](/developers/data-schema/connector-index)** - Connector-specific traversal considerations
- Review [Object Schema](/developers/data-schema/object-schema), [Geometry Schema](/developers/data-schema/geometry-schema), [Proxy Schema](/developers/data-schema/proxy-schema) for details
