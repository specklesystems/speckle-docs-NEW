...
<Accordion title="How do I find all elements on a specific floor?">
  Filter by the `level` field on ArchicadObjects, or traverse the collection hierarchy which
  organizes objects by floor. The `level` field is a string matching the floor name.
</Accordion>
