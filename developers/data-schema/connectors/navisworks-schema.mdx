...
<Info>
  The Speckle `NavisworksObject` represents the **first selectable ancestor** `Navisworks.ModelItem`
  object in Navisworks.
</Info>
...
<Accordion title="What does 'first selectable ancestor' mean?">
  In Navisworks, the selection tree can have nested items where parent items aren't directly
  selectable. The NavisworksObject represents the first item in the hierarchy that can actually be
  selected, which may be a parent of the actual geometry. This ensures objects are meaningful and
  selectable entities.
</Accordion>
