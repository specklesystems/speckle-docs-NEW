...
<Accordion title="What's the difference between Section and Material proxies?">
  ...
  **Material**: Defines the material properties (e.g., Steel A36, Concrete 3000psi). Material
  proxies reference section proxies to create material-section combinations.
</Accordion>

<Accordion title="How do I access analysis results?">
  Analysis results are stored in the Root Collection's `info.analysisResults` field. This is
  model-level data, not per-object. Access it from the root collection object, not from individual
  EtabsObjects.
</Accordion>
