...
<Accordion title="How do I find all walls on a specific level?">
  You can filter by the `level` field on RevitObjects, or use Level proxies. The `level` field is a
  string matching the level name. You can also traverse the collection hierarchy which organizes
  objects by level.
</Accordion>
