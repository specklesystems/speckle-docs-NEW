...
Use the Instance's `definitionId` to find the corresponding Definition proxy, then access `proxy.value.displayValue` to get the geometry. See [Traversal Recipes](/developers/data-schema/traversal-recipes) for a complete example.
</Accordion>
