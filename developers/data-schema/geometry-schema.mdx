...
```mermaid
graph TD
    A[DataObject] --> B["name: 'Wall A1'"]
    A --> C["properties: {...}"]
    A --> D["displayValue: array"]
    D --> E[Mesh<br/>Visual representation]
    D --> F[Line<br/>Edge curves]
    D --> G[Point<br/>Key points]
```
...
```mermaid
graph TD
    A[Mesh] --> B["vertices: [x1, y1, z1, ...]<br/>Flat array of coordinates<br/>every 3 values = one point"]
    A --> C["faces: [3, 0, 1, 2, ...]<br/>Each face preceded by vertex count<br/>Example: [3, 0, 1, 2] = triangle<br/>Example: [4, 0, 2, 3, 4] = quad"]
    A --> D[Optional fields]
    D --> E["colors: [r1, g1, b1, ...]<br/>Colors per vertex RGB<br/>Faces take average<br/>Used for quasi-textures"]
    D --> F["normals: [nx1, ny1, nz1, ...]<br/>Vertex normals<br/>Stored but not heavily used"]
    D --> G["textureCoordinates: [...]<br/>Not yet utilized"]
```
