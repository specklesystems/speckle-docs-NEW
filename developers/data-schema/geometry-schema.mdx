...
│   └── Flat array of coordinates (every 3 values = one point)
│
├── faces: [3, 0, 1, 2, 4, 0, 2, 3, 4, ...]
│   └── Each face is preceded by its vertex count (supports ngons)
│       └── Example: [3, 0, 1, 2] = triangle with vertices 0, 1, 2
│       └── Example: [4, 0, 2, 3, 4] = quad with vertices 0, 2, 3, 4
│
└── Optional fields:
    ├── colors: [r1, g1, b1, r2, g2, b2, ...]
    │   └── Colors per vertex (RGB values)
    │   └── Faces take the average of their vertex colors
    │   └── Used for quasi-textures or analytical results visualization
    │
    ├── normals: [nx1, ny1, nz1, nx2, ny2, nz2, ...]
    │   └── Vertex normals (stored but not heavily used in connectors due to lack of host support)
    │
    └── textureCoordinates: [...]
        └── Not yet utilized
```
