...
1. **No nested DataObjects** - DataObjects cannot contain other DataObjects as direct children. Use `displayValue` for geometry, and use proxies for relationships. *Exception: Revit curtain walls and IFC data may use an `elements` property (v2 style) to contain child objects.*
