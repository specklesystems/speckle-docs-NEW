...
```mermaid
graph TD
    A[Object] --> B["id: content-based hash<br/>unique identifier"]
    A --> C["speckle_type: type name<br/>e.g., 'Objects.Data.DataObject'"]
    A --> D["applicationId: source application identifier"]
    A --> E["units: unit system<br/>e.g., 'meters'"]
    A --> F["name: human-readable name<br/>for DataObjects"]
    A --> G["properties: dictionary<br/>key-value pairs for DataObjects"]
    A --> H["displayValue: array<br/>geometry objects for DataObjects"]
```
...
```mermaid
graph TD
    A[DataObject] --> B["displayValue: array"]
    B --> C[Mesh<br/>visual representation]
    B --> D[Line<br/>edge curves]
```
