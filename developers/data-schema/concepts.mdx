...
A version reaches its Root Collection through the `referencedObject` id on the version record. The version record holds publish metadata (message, author, source application, timestamp); the Root Collection holds the package (name, `units`, `elements`, proxy arrays, info). See [Versions and the root object](/developers/data-schema/version-root-object) for the full split and how to read each in a script.
