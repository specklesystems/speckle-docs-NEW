...
```json
{
  ...
  "name": "string",
  "value": { "resource data" },
  "objects": [ "applicationId strings" ]
}
```

- **Location**: Proxies are at Root Collection level
- **References**: Use `applicationId`, never `id`
- **Resolution**: Find proxy by `applicationId`, check `objects` array
...
def find_objects_using_resource(root, proxy_type, resource_name):
    ...
    if not proxy:
        return []
    # Get referenced IDs
    referenced_ids = getattr(proxy, 'objects', [])
    # Find objects
...
| Resolve material | Find RenderMaterial proxy, check `objects`                        |
...
- `value` (resource data)
- `objects` (array of `applicationId` strings)
...
- [Object Schema](/developers/data-schema/object-schema) - Detailed data model
