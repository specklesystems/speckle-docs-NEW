...
<CardGroup cols={2}>
  <Card title="Use Personal Access Tokens" icon="key">
    When: - Single-user tools or scripts - Server-side applications - Automation and CI/CD -
    Personal development projects
  </Card>
  <Card title="Use OAuth2 with PKCE" icon="shield">
    When: - Multi-user web applications - Public-facing apps - Third-party integrations - Apps
...
<Check>
  ...
  needed - Use secure storage (keychain, secret managers) in production - Implement token refresh
  for OAuth2 tokens
</Check>

<Warning>
  Don't: - Commit tokens to version control - Share tokens between team members - Use PATs in
  client-side code (use OAuth instead) - Store tokens in plain text files - Use overly permissive
  scopes - Expose app secrets in client-side code - Log tokens in application logs
</Warning>
