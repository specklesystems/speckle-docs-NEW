...
<AccordionGroup>
  ...
  <Accordion title="How do I find my project ID?">
    ...
    - **Web app:** Open the project and copy the id segment from the URL.
    - **GraphQL:** Run any `project(id: "…") { id }` query (or use Step 1 once you have a candidate id).
    - **SpecklePy:** List projects with [`workspace.get_projects`](/developers/sdks/python/api-reference/resources/workspace#get-projects) or [`active_user.get_projects`](/developers/sdks/python/api-reference/resources/active-user#get-projects), depending on whether the server is workspace-enabled (see [SpeckleClient → Resource Properties](/developers/sdks/python/api-reference/client#resource-properties)). Load one project with [`project.get`](/developers/sdks/python/api-reference/resources/project#get).
    - **Sync row ids:** Read `items[].id` from the Step 1 `project.syncs` query. SpecklePy does not ship typed sync list helpers yet; use [`execute_query`](/developers/sdks/python/api-reference/client#custom-graphql-queries) with the same document as Step 1, or reuse the curl example above.
  ...
  <Accordion title="Can I automate sync updates with SpecklePy?">
    Yes. The Speckle Python SDK does not ship dedicated methods for sync mutations yet, but you can run the same GraphQL operations as this guide. Authenticate with `authenticate_with_token` using a PAT that includes `streams:read` and `streams:write`, then use raw GraphQL via `SpeckleClient.execute_query()` with the [`gql`](https://pypi.org/project/gql/) helper as described in [Custom GraphQL Queries](/developers/sdks/python/api-reference/client#custom-graphql-queries). Alternatively, POST JSON bodies to `{host}/graphql` from Python with `requests` or `httpx`, matching the `curl` example above.
  </Accordion>
</AccordionGroup>
