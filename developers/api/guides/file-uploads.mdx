...
<Accordion title="Why are different formats supported in self-hosted vs cloud?">
  ...
  <Accordion title="Where did the StartFileImport mutation go?">
    Gone, not just deprecated. `startFileImport` is replaced by
    ...
    fails outright with a 400 error (`UNSUPPORTED_FILE_TYPE`) rather than falling back to the old
    behavior — there's no compatibility window for this one.
