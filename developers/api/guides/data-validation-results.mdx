...
Use one consistent example set across this guide. Copy the segment after each path prefix from your browser URL, or read `id` fields from the discovery query below.

| ID                     | Example value | Full URL pattern                                                              |
| ---------------------- | ------------- | ----------------------------------------------------------------------------- |
| `projectId`            | `0d1f93ec13`  | `https://app.speckle.systems/projects/0d1f93ec13/data-validation`             |
| Check ID (`insightId`) | `abc456def7`  | `https://app.speckle.systems/projects/0d1f93ec13/data-validation/abc456def7/` |
| `modelId`              | `f2a8b1c9d0`  | `https://app.speckle.systems/projects/0d1f93ec13/models/f2a8b1c9d0`           |
| `versionId`            | `7e3c21a8b4`  | From `latestResults[].versionId` or the model version URL                     |

Replace example values with IDs from your project when you run queries against live data.
...
Request the full `result` payload for one model's history. Results are ordered newest first.
