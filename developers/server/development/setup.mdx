...
<Warning>
  **Development vs Production**: This setup is for development and testing only. For production
  deployment, use the [Docker Compose Quickstart](/developers/server/getting-started) or
  [Kubernetes](/developers/server/deployment/kubernetes-with-helm) guides.
</Warning>
...
<Warning>
  Don't forget to set up the variables in the `.env` & `.env.test` files according to your
  deployment
</Warning>
