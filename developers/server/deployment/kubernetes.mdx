...
<Frame>
  <img src="/images/developers/server/kubernetes/27_add_speckle_helm_repo.png" />
</Frame>
...
<Frame>
  <img src="/images/developers/server/kubernetes/28_helm_repo_update.png" />
</Frame>
...
Navigate to your domain registrar's website for your domain name and add a DNS A record. This will allow web browsers to resolve your domain name to the IP of the load balancer. The domain must match the domain name provided to Speckle in the `values.yaml` file you edited previously. If DigitalOcean manages your Domain Names, adding a DNS A record using [DigitalOcean's Domain page](https://cloud.digitalocean.com/networking/domains) will look something like the following:
<Frame>
...
In this case, our deployment is correct but our browser rightly does not trust Let's Encrypt's staging environment. To resolve this issue, we recommend amending the Certificate to a production certificate. Please refer above for notes on how to amend your Speckle deployment to use Let's Encrypt's Production environment.
