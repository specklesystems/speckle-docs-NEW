...
description: Complete guide to backing up, upgrading, and restoring Speckle server databases. Learn how to set up pgAdmin, create backups, perform upgrades, and restore data using Docker Compose.
---
...
<Steps>
  <Step title="Backup database">
    Connect to pgAdmin, then connect to your Postgres server. Right-click on the `speckle` database,
    select `Backup...`, choose a location, and click `Backup` to start the process.
  </Step>
</Steps>
...
<Step title="Verify deployment">Check that all containers are running: ```bash docker ps```</Step>
