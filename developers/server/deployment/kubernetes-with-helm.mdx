...
<Note>
  This walkthrough installs the public open-source chart from Docker Hub, which follows the public
  `speckle-server` repository (currently the 2.31 line). Speckle Enterprise Server installs a
  different chart at a different version from a private registry: use this page for the cluster,
  dependencies, ingress and certificates, and the [Enterprise deployment
  guide](/developers/server/deployment/enterprise-license) for the chart, its version, and the
  additional secrets and values it needs.
</Note>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/01_select_node_size.png"
    alt="01 select node size"
  />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/02_kubernetes_clusters.png"
    alt="02 kubernetes clusters"
  />
</Frame>
...
<Frame>
  <img src="/images/developers/server/kubernetes/03_get_kubeconfig.png" alt="03 get kubeconfig" />
</Frame>
...
<Frame>
  <img src="/images/developers/server/kubernetes/04_show_contexts.png" alt="04 show contexts" />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/05_kubectl_get_nodes.png"
    alt="05 kubectl get nodes"
  />
</Frame>
...
Speckle requires a Valkey cache to function. You can provide your own if you have an existing database. This should be a single-node Valkey cache, not a multi-node cluster. Otherwise, follow the steps below to create a new Valkey database on DigitalOcean.
...
- We will deploy a managed Valkey provided by DigitalOcean. Go to the [new Database creation page](https://cloud.digitalocean.com/databases/new). Firstly, select the same region and VPC as you used when deploying your Kubernetes cluster, and select Valkey. Provide a name, and click `Create Database Cluster`.
  ...
  <Frame>
    <img
      src="/images/developers/server/kubernetes/06_redis_configuration.png"
      alt="06 redis configuration"
    />
  </Frame>
...
- From the overview, click on `Secure this database cluster by restricting access.` This will take you to the Trusted Sources panel in the Settings tab. Here, we will improve the security of your database by only allowing connections from your Kubernetes cluster. Type the name of your Kubernetes cluster and add it as a Trusted Source.
  ...
  <Frame>
    <img
      src="/images/developers/server/kubernetes/07_redis_trusted_source.png"
      alt="07 redis trusted source"
    />
  </Frame>

- In the Overview tab for your Valkey database. Select `connection string` from the dropdown, and copy the displayed Connection String. You will require this when configuring your deployment in [step 4](#configure-your-deployment).
  <Frame>
    <img
      src="/images/developers/server/kubernetes/08_redis_connection_string.png"
      alt="08 redis connection string"
    />
  </Frame>
...
- We will now deploy a managed Postgres provided by DigitalOcean. Go to the [new Database creation page](https://cloud.digitalocean.com/databases/new). Firstly, select the same region and VPC as you used when deploying your Kubernetes cluster, and then select Postgres. Provide a name, and click `Create Database Cluster`.
  ...
  <Frame>
    <img
      src="/images/developers/server/kubernetes/09_postgres_configuration.png"
      alt="09 postgres configuration"
    />
  </Frame>
  <Frame>
    <img
      src="/images/developers/server/kubernetes/09_postgres_configuration_02.png"
      alt="09 postgres configuration 02"
    />
  </Frame>
...
- From the overview page for your Postgres database, click on `Secure this database cluster by restricting access`. This will take you to the Trusted Sources panel in the Settings tab. Here, we will improve the security of your database by only allowing connections from your Kubernetes cluster. Type the name of your Kubernetes cluster and add it as a Trusted Source.
  ...
  <Frame>
    <img
      src="/images/developers/server/kubernetes/10_postgres_trusted_source_edit.png"
      alt="10 postgres trusted source edit"
    />
  </Frame>

- In the Overview tab for your Valkey database. Select `connection string` from the dropdown, and copy the displayed Connection String. You will require this for when configuring your deployment in [step 4](#configure-your-deployment).
  <Frame>
    <img
      src="/images/developers/server/kubernetes/11_postgres_connection_string.png"
      alt="11 postgres connection string"
    />
  </Frame>
...
- Navigate to the [Create a Space page](https://cloud.digitalocean.com/spaces/new). Please select a region of your choice, we recommend the same region as you have deployed the cluster. We did not enable the CDN, and we restricted the file listing for security purposes. Please provide a name for your Space, this has to be unique in the region so please use a different name than our example. Make a note of this name; this is the `bucket` value, which we will require when configuring your deployment in subsequent steps. Click on `Create Space`.
  ...
  <Frame>
    <img
      src="/images/developers/server/kubernetes/12_blob_storage_configuration.png"
      alt="12 blob storage configuration"
    />
  </Frame>

- Once created, click on the `Settings` tab and add a `CORS Configurations`.

- Add a CORS Configuration which allows `GET`, `PUT` and `POST` requests from your domain, allows all request headers, and exposes the `ETag`, `Content-Encoding` and `Content-Length` response headers. Browsers and connectors upload and download model data directly against the bucket, and `ETag` is required to complete an upload.

<Frame>
  <img
    src="/images/developers/server/kubernetes/38_spaces_cors_configuration.png"
    alt="38 spaces cors configuration"
  />
</Frame>
...
- Now click on the `Settings` tab and copy the `Endpoint` value.
  ...
  <Frame>
    <img
      src="/images/developers/server/kubernetes/13_spaces_endpoint.png"
      alt="13 spaces endpoint"
    />
  </Frame>
...
- Now navigate to the [API page](https://cloud.digitalocean.com/account/api/tokens) in DigitalOcean. Next to the `Spaces access keys` heading, click `Generate New Key`. You will only be able to see the Secret value once, so copy the name, the key and the secret and store this securely.
  <Frame>
    <img
      src="/images/developers/server/kubernetes/14_spaces_access_key.png"
      alt="14 spaces access key"
    />
  </Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/15_create_namespace.png"
    alt="15 create namespace"
  />
</Frame>
...
<Frame>
  <img src="/images/developers/server/kubernetes/16_get_namespaces.png" alt="16 get namespaces" />
</Frame>
...
<Frame>
  <img src="/images/developers/server/kubernetes/17_secrets.png" alt="17 secrets" />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/18_priority_classes.png"
    alt="18 priority classes"
  />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/19_helm_jetstack_repo.png"
    alt="19 helm jetstack repo"
  />
</Frame>
...
Deploy the CertManager Helm release with the following command. Replace `${YOUR_CLUSTER_CONTEXT_NAME}` with the name of your cluster. This installs the latest cert-manager release; check that it supports your cluster's Kubernetes version in the [cert-manager release table](https://cert-manager.io/docs/releases/), and add `--version` to pick an older release if it does not.

```shell
helm upgrade cert-manager jetstack/cert-manager --namespace cert-manager --set crds.enabled=true --install --create-namespace --kube-context "${YOUR_CLUSTER_CONTEXT_NAME}"
```

<Frame>
  <img src="/images/developers/server/kubernetes/20_certmanager.png" alt="20 certmanager" />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/21_certmanager_pods.png"
    alt="21 certmanager pods"
  />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/22_certmanager_cluster_issuer.png"
    alt="22 certmanager cluster issuer"
  />
</Frame>
...
We can verify that this worked by running the following command. Replace `${YOUR_CLUSTER_CONTEXT_NAME}` with the name of your cluster. The response should state that the message was "_The ACME account was registered with the ACME server_".
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/23_certmanager_account_registered.png"
    alt="23 certmanager account registered"
  />
</Frame>
...
We repeat this command to verify that the production certificate was created as well. Replace `${YOUR_CLUSTER_CONTEXT_NAME}` with the name of your cluster. Again, the response should state that the message was "_The ACME account was registered with the ACME server_".
...
<Frame>
  <img src="/images/developers/server/kubernetes/24_helm_nginx_repo.png" alt="24 helm nginx repo" />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/25_helm_repo_update.png"
    alt="25 helm repo update"
  />
</Frame>
...
Now we can deploy NGINX to our kubernetes cluster. The additional annotation allows CertManager, deployed in the [previous step](#certificate-manager), to advise NGINX which certificate to use for https connections. Replace `${YOUR_CLUSTER_CONTEXT_NAME}` with the name of your cluster.
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/26_ingress_controller.png"
    alt="26 ingress controller"
  />
</Frame>

<Note>
  We can ignore the instructions printed out by the NGINX Helm chart as the required resources will
  be provided by the Speckle Helm chart.
</Note>
...
- Fill in the requested fields and save the file:
  ...
  - `s3.publicEndpoint`: required when `s3.endpoint` is a private or in-cluster address. Browsers and connectors fetch and upload model data through pre-signed URLs built on this value, so it must be an HTTPS URL reachable from your users' machines. Leave it empty when `s3.endpoint` is already public, as with DigitalOcean Spaces.
  ...
  - `server.auth.local.enabled`: this is enabled by default. This requires users to register on your Speckle cluster with a username and password. If you wish to use a different authorization provider, such as Azure AD, GitHub, or Google, set this value to `false` and amend the relevant section below by enabling it and providing the necessary details.
  - `server.email`: optional, enabling emails will enable extra features like sending invites.
    ...
    - This also requires the `email_password` secret to have been set in [Step 3](#create-secrets).
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/37_values_yaml_showing_certificate_formatting.png"
    alt="37 values yaml showing certificate formatting"
  />
</Frame>
...
Run the following command to deploy the Helm chart to your Kubernetes cluster configured with the values you configured in the [prior step](#configure-your-deployment). Replace `${YOUR_CLUSTER_CONTEXT_NAME}` with the name of your cluster.

```shell
helm upgrade my-speckle-server oci://registry-1.docker.io/speckle/speckle-server-chart \
 --values values.yaml \
 --namespace speckle \
 --install --create-namespace \
 --kube-context "${YOUR_CLUSTER_CONTEXT_NAME}"
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/29_install_speckle_release.png"
    alt="29 install speckle release"
  />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/30_helm_release_verification.png"
    alt="30 helm release verification"
  />
</Frame>
...
<Frame>
  <img src="/images/developers/server/kubernetes/31_get_IP_address.png" alt="31 get IP address" />
</Frame>
...
<Frame>
  <img src="/images/developers/server/kubernetes/32_domain_a_record.png" alt="32 domain a record" />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/33_registration_page.png"
    alt="33 registration page"
  />
</Frame>
...
<Frame>
  <img src="/images/developers/server/kubernetes/34_browser_warning.png" alt="34 browser warning" />
</Frame>
...
<Frame>
  <img
    src="/images/developers/server/kubernetes/35_inspecting_certificates.png"
    alt="35 inspecting certificates"
  />
</Frame>
<Frame>
  <img
    src="/images/developers/server/kubernetes/36_certificate_details.png"
    alt="36 certificate details"
  />
</Frame>
