...
<Note>
  When a federated model view is generated from the a model folder, additional
  project models can be added to the federation, but none removed from the view.
  Instead either assemble a federation by adding models one at a time, or use
  the hide and isolation tools.
</Note>
