...
<Note>
**Webhooks vs Subscriptions**: Webhooks are HTTP-based and ideal for server-to-server integrations. For client applications (web apps, desktop apps) that need real-time UI updates, consider using [GraphQL Subscriptions](/developers/api/subscriptions) instead, which use WebSocket connections.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Can I use webhooks on Speckle Cloud?">
    Yes! Webhooks are available on any Speckle server, including [app.speckle.systems](https://app.speckle.systems) and self-hosted instances. You don't need to deploy your own server to use webhooks.
  </Accordion>
