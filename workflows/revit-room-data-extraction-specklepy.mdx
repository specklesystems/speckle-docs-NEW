...
In this workflow, we will demonstrate how to extract room data from Revit models using SpecklePy SDK.
...
def extract_object_data(objects, field_paths, include_defaults=True):
    ...
    Parameters:
    objects (list): List of DataObjects (Speckle Base objects).
    field_paths (dict): Mapping of output column name -> path list within obj.properties.
        Each path is a list of dict keys to traverse, e.g.:
            {
    ...
    include_defaults (bool): If True, prepend id, applicationId, and object name columns.
