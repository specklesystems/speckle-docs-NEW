...
<Info>
  This notebook currently targets Revit-oriented analytics paths (for example `category` and
  `proxy.level` examples in EAV queries).
</Info>
...
<Warning>
  Older published versions created before automatic dataset generation may return availability or
  download errors. In that case, run this tutorial on a newer published version.
</Warning>
...
- Main packfile: `/api/v1/projects/{projectId}/models/{modelId}/versions/{versionId}/download`
- EAV packfile: `/api/v1/projects/{projectId}/models/{modelId}/versions/{versionId}/eav/download`
...
# Minimal input from env
# Example: SPECKLE_MODEL_URL=https://app.speckle.systems/projects/{projectId}/models/{modelId}@{versionId}
...
if "@" in model_ref:
    ...
    model_id, version_id = model_ref, None

# If version is missing, resolve latest version first
if not version_id:
    latest_query = """
...
"""
    ...
    if latest_payload.get("errors"):
        raise RuntimeError(latest_payload["errors"])

    items = latest_payload["data"]["project"]["model"]["versions"]["items"]
    if not items:
...
result = payload["data"]
version = result["project"]["model"]["version"]
if not version:
    raise RuntimeError("Version not found.")
if not version.get("objectKey"):
    raise RuntimeError(
        "Primary dataset not available for this version. "
        "This often happens on older published versions created before auto-generation."
    )

print(f"Resolved IDs: project={project_id}, model={model_id}, version={version_id}")
```

If you prefer, you can still use `client.execute_query(query)` for queries without variables.

```python
# Example for simple no-variables query
simple_query = "{ activeUser { id name } }"
simple_result = client.execute_query(simple_query)
```
...
with requests.get(main_url, headers=headers, stream=True, timeout=300) as r:
    r.raise_for_status()
...
with requests.get(eav_url, headers=headers, stream=True, timeout=300) as r:
    r.raise_for_status()
    with eav_path.open("wb") as f:
        for chunk in r.iter_content(chunk_size=1024 * 1024):
            if chunk:
                f.write(chunk)
```
...
### 5) Run practical analytics queries
...
Get top levels by object count:
...
[Download the notebook](https://raw.githubusercontent.com/specklesystems/speckle-docs-new/refs/heads/main/workflows/notebooks/specklepy-packfiles-analytics.ipynb).
Save it locally, add your `.env` in the same folder, and run top to bottom.

Notebook default path is now:

- set `SPECKLE_TOKEN`
- set one of:
  - `SPECKLE_MODEL_URL` (recommended), or
  - `SPECKLE_PROJECT_ID` + `SPECKLE_MODEL_ID` (+ optional `SPECKLE_VERSION_ID`)

## Troubleshooting

<AccordionGroup>
  <Accordion title="404 on dataset download">
    Check `projectId`, `modelId`, and `versionId` first. Then confirm the version has `objectKey` in
    GraphQL metadata.
  </Accordion>
