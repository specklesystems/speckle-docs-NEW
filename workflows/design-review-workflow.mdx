...
description: "Run structured design reviews in Speckle using saved views, federation, markups, discussions, and issues."
---

Use this guide when you need a repeatable way to run design reviews in Speckle. It ties together saved views, federation, markups, discussions, and issues into one end‑to‑end workflow. For teams that already use Speckle for viewing and sharing models.
...
<Steps>
  ...
  <Step title="Convert important points into issues">
    For each decision or required change, create an **Issue** on the relevant object(s). Set title, description, assignee, priority, and due date while the discussion is fresh. Use labels to tag discipline or
  ...
  <Step title="Use discussions for open questions">
    When there is an open question or debate that will continue after the meeting, start a **Discussion** instead of an issue. Mention the relevant people, summarise options, and link to any supporting files.
    See [Discussions](/3d-viewer/discussions) for collaboration details.

    <Frame>
      <img src="/images/3d-viewer/discussions-side-panel.jpg" alt="Discussions panel with threaded comments alongside the model" />
    </Frame>
  </Step>
...
Outcome: Key decisions and change requests are captured as issues and discussions, with visual markups and saved views tying them back to the 3D model.
...
<Steps>
  ...
  <Step title="Use discussions for broader topics">
    For topics that affect multiple areas or models (e.g. fire strategy, circulation), continue the conversation in **Discussions**. Encourage people to reply in threads so context stays in one place.
  </Step>
  <Step title="Assign clear owners">
    Ensure every actionable comment is either an **Issue** with an assignee or a Discussion with named people using @mentions. This prevents feedback from being lost in general threads.
  </Step>
  <Step title="Resolve and hide completed discussions">
    When a topic is agreed or a question is answered, resolve the discussion so it no longer clutters the list. You can always re‑open it later if required.
  </Step>
</Steps>

Outcome: Feedback from people who were not in the original meeting is captured directly against the model and grouped by topic.
...
- **Avoid duplicate channels**: Encourage feedback in issues and discussions instead of email and chat so history stays with the model.
...
<Accordion title="Should we use issues or discussions for feedback?">
Use issues for actionable work items with clear owners and due dates. Use discussions for broader topics, questions, or design debates that may not have a single task attached.
</Accordion>
...
Yes. Give them view‑only access to the project and share links to saved views. They can still open issues and discussions if project permissions allow, but they cannot change the model geometry.
</Accordion>
<Accordion title="What if we forget to create issues during the meeting?">
You can create issues after the fact from markups or discussions by revisiting the saved views and translating important comments into issues. The earlier you do this, the less context you lose.
</Accordion>
...
- [Discussions](/3d-viewer/discussions)
