...
When multiple sources are present, validation recalculates only for selected loaded sources. This keeps results focused
on what you are actively reviewing and improves responsiveness.
...
<AccordionGroup>
<Accordion title="Why does validation run only on some sources?">
Validation recalculates only for selected loaded sources. This is expected behavior and helps avoid recomputing models
that are not currently in scope. If you need additional sources included, load and select them, then run validation
again.
</Accordion>
