...
<AccordionGroup>
  <Accordion title="Should I start with a check or a standard?">
    **Check** for one live requirement against models today. **Standard** when several
    checks should share predicates and wording.
  </Accordion>
  <Accordion title="Do checks run automatically?">
    Per check: manual reruns only, or automatic when a tracked model publishes a new
    version.
  </Accordion>
  <Accordion title="Where do I inspect detailed outcomes?">
    Open the check’s latest/history tabs. Screen layout lives on [Viewing
    Results](/analytics/data-validation/viewing-results).
  </Accordion>
  <Accordion title="Can I still use dashboard widgets for validation?">
    Yes: [Validation Widgets in Dashboards](/analytics/dashboards/validation-widgets)
    (still in draft).
  </Accordion>
</AccordionGroup>
