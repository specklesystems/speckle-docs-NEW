...
<Note>
  On 2026.9, a new check starts with an empty object scope, some property paths change, and a rule
  can check an object against a related object such as a host wall. **Equals property** comparisons
  can map labels before they compare. Existing checks that return empty results may need rebuilding.
  See [Data Validation in 2026.9](/next/analytics/data-validation/overview).
</Note>

<Note>
  2026.9 does not include **Project standards**. If your team already built them, email
  [hello@speckle.systems](mailto:hello@speckle.systems). See [Project standards in
