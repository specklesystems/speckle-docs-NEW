...
<Warning>
  **Calculations that run across many elements are easy to misread.** Most calculated
  fields run **once per element**. Charts and totals that **Sum** those results can
  multiply a ratio or an already-totalled value by the element count — so a
  “percentage” can look like hundreds of thousands, and a total can look impossibly
  large. Before you trust a number: ask whether each step is one dashboard-level figure
  (**Dashboard constant**) or a value stamped on every object; prefer constants (or
  literals) for shared denominators; and use
  [Things to watch out for](#things-to-watch-out-for) and
  [Debugging and validating values](#debugging-and-validating-values) when something
  looks off.
</Warning>
