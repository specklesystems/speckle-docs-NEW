...
<Note>
Ruleset **TSV** is a legacy interchange format; Speckle is moving away from it. If you still use TSV files, their
**Predicate** column uses different wording than the UI (for example `equal to` instead of **Equals**, `in list` instead
of **Is one of**, `is like` instead of **Matches pattern**). See
[Rules and Rulesets — TSV predicate strings](/analytics/data-validation/rules-and-rulesets#tsv-predicate-strings).
</Note>
...
<Note>
For numbers, the menu lists both **Equals** / **Does not equal** and **Is equal to** / **Is not equal to**. All are
equality-style checks; use the option that matches how the UI asks for values for your rule or filter.
</Note>
...
- **Number**: Equality, inequality, greater than, less than, and between min and max
...
it as text (for example **Contains** or **Equals**). For numeric comparisons (greater than, less than, between), the
value is interpreted as a number when possible. Boolean properties use **Is true** and **Is false**.
