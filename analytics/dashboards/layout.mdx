...
**Change Data Source** connects a data widget to another model already loaded on
...
- **Model Viewer** and **Dual viewer** are excluded — their models are managed separately. Change models
...
<Steps>
  <Step title="Add sections to group areas">
    ...
    "Validation"). Add widgets inside each section so related content appears together.
  </Step>
  <Step title="Resize widgets">
    Drag a widget edge or corner until its content fits. The widget displays at the new size.
  </Step>
...
{/* IMAGE_PLACEHOLDER: Dashboard with Section widgets grouping areas */}
...
<AccordionGroup>
  ...
  <Accordion title="Is there a limit to how many widgets I can add?">
    There's no hard limit for most widgets, but dashboard performance may decrease with very large
    numbers of widgets. **3D viewer widgets are the exception** — see "Is there a limit to how many
    3D viewer widgets I can add?" below.
  </Accordion>
  <Accordion title="Is there a limit to how many 3D viewer widgets I can add?">
    Yes. Widgets that render a 3D viewer or map each use one or more browser rendering contexts —
    even an empty one with no model assigned yet. Browsers cap how many a page can hold open at once
    (typically 16); once a dashboard reaches that limit, adding another such widget is blocked. This
    is a browser-level limit, not a Speckle setting. Remove widgets you're not using to stay under
    it.
  </Accordion>
  <Accordion title="Why do some of my viewer or map widgets show a broken-image icon instead of loading?">
    This usually means the dashboard already has more 3D viewer or map widgets than your browser
    allows open at once, for example on a dashboard created before the limit was enforced. Adding a
    widget is blocked once you're at the limit, but opening a dashboard that's already over it still
    loads every widget, so the extras fail instead. Remove the widgets you don't need and reload the
    dashboard to fix it.
  </Accordion>
  ...
  <Accordion title="How many models can I add to one dashboard?">
    There are no hard limits on data sources themselves, but see "Why is my dashboard slow?"—large
    numbers of models or widgets can affect performance. If those models are shown in 3D viewer
    widgets rather than just charts and tables, see "Is there a limit to how many 3D viewer widgets
    I can add?" above.
  </Accordion>
  ...
  <Accordion title="If I pin a version, does the dashboard ever change?">
    ...
    or unpin](/analytics/intelligence-dashboards#change-a-pinned-version) a loaded model without
    creating a new data source — widgets stay connected to that data source. The change saves the
    dashboard immediately. Charts and tables recompute against the new version’s properties; paths
    that only existed on the old version show no values until you select a property from the new
    version.
  </Accordion>
...
(copies the active palette). Project and workspace sections offer **New theme from
current** when you have permission. To customize a built-in, select it, create a copy in the appropriate
