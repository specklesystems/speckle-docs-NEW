...
<AccordionGroup>
  ...
  <Accordion title="Is there a limit to how many widgets I can add?">
    There's no hard limit for most widgets, but dashboard performance may decrease with very
    large numbers of widgets. **3D viewer widgets are the exception** — see "Is there a limit
    to how many 3D viewer widgets I can add?" below.
  </Accordion>
  <Accordion title="Is there a limit to how many 3D viewer widgets I can add?">
    Yes. Model Viewer counts as one browser 3D rendering context, Dual viewer as two (one per
    pane), and Map's 3D layer as one — even for an empty viewer with no model assigned yet.
    Browsers cap how many of these a page can hold open at once (typically 16); once a
    dashboard reaches that limit, adding another viewer widget is blocked. This is a
    browser-level limit, not a Speckle setting. Remove viewer widgets you're not using to stay
    under it.
  </Accordion>
  ...
  <Accordion title="How many models can I add to one dashboard?">
    There are no hard limits on data sources themselves, but see "Why is my dashboard
    slow?"—large numbers of models or widgets can affect performance. If those models are
    shown in 3D viewer widgets rather than just charts and tables, see "Is there a limit to
    how many 3D viewer widgets I can add?" above.
  </Accordion>
