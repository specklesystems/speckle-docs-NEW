...
<Steps>
  ...
  <Step title="Add Element count for total elements">
    ...
    filter by level, category, or family.
  </Step>
  <Step title="Add a breakdown widget">
  ...
  <Step title="Filter the dashboard">
    Click a value in any chart or table to filter the whole dashboard (e.g. one level, one
    category).
  </Step>
  <Step title="Browse elements in a table">
    Add **Element table** to see rows; customise columns, filter, and sort. Filter state is saved
    with the dashboard.
  </Step>
</Steps>

{/* IMAGE_PLACEHOLDER: Dashboard with Model Viewer and project/model selected */}

{/* IMAGE_PLACEHOLDER: Filter chips at top and chart/table with value clicked */}

Outcome: Counts and breakdowns tied to the 3D model; filter state saved with the dashboard.

<Tip>
  On 2026.9, you can ask for a priced takeoff in plain language instead — see [Take off quantities
  and cost with Speckle Intelligence](/next/workflows/take-off-quantities-with-intelligence).
</Tip>
...
<Tip>
  Use Aggregate + Arithmetic for shares. An **All data** or **Current view** Aggregate field in a
  **Ratio** widget can inflate the result. See [Things to watch out
  for](/analytics/dashboards/calculated-fields#things-to-watch-out-for).
</Tip>

For comparisons between two models, use [Dashboard constants](/analytics/dashboards/calculated-fields#dashboard-constants).
To correct area units, create a **Unit conversion** field and use it in both calculations.

<div id="show-an-x-per-y-number-from-two-models" />

## Compare totals from two models
...
you want that figure to remain fixed when someone filters the charts.
...
<Steps>
  ...
  <Step title="Create the comparison constants">
...
Outcome: One clear intensity figure such as load per m². It does not change when
...
<Steps>
  <Step title="Load both model versions">
  ...
  <Step title="Synchronize the cameras">
...
<Steps>
  <Step title="Load the site and design schemes">
  ...
  <Step title="Connect widgets to design schemes">
...
<Steps>
  <Step title="Load the projects or models">
  ...
  <Step title="Group widgets with sections">
...
{/* IMAGE_PLACEHOLDER: Dashboard with multiple Model Viewers or data source selector */}

Outcome: One dashboard for portfolio-level counts and comparisons; stakeholders see the same view.

<Tip>
  On 2026.9, if you want to compare one project against a benchmark library rather than watch
  several live projects at once, see [Benchmark a project against your portfolio with Speckle
  Intelligence](/next/workflows/benchmark-against-your-portfolio-with-intelligence).
</Tip>
...
<Steps>
  ...
  <Step title="Add a reusable ruleset">
...
<Steps>
  <Step title="Add a model breakdown widget">
  ...
  <Step title="Save a reusable validation rule">
  ...
  <Step title="Pin the model version">Pin the model version so the result is reproducible.</Step>
</Steps>

{/* IMAGE_PLACEHOLDER: Filter UI with Matches pattern operator and wildcard value */}
...
<Steps>
  ...
  <Step title="Add further calculated fields">
...
<Note>
  This workflow adds a Model Viewer, Context, and optionally Map to the same dashboard — each uses
  one or more browser rendering contexts. On a dashboard that already has several viewer or map
  widgets, you can hit your browser's limit. See [3D viewer widget
  limit](/analytics/dashboards/layout#is-there-a-limit-to-how-many-3d-viewer-widgets-i-can-add).
</Note>
...
<Steps>
  ...
  <Step title="Set the dashboard site location">
    ...
    rotation). Select **Apply** to save the placement. The site location is owned by the dashboard —
    you do not need geographic data on the model. You can also set the site from a **Map** widget;
    both share the same site.
  </Step>
  <Step title="Add a map for location pins">
  ...
  <Step title="Tune filters, colour, and the map">
    ...
    boundary**. **Clip model to ground** hides Speckle geometry below the map surface (on by
    default).
  </Step>
</Steps>

{/* IMAGE_PLACEHOLDER: Context widget showing model on pitched 3D map */}
...
The **Map** widget shows site markers and location pins. Use **Context** to show model geometry
on the map. The Model Viewer keeps its local coordinates. Context also saves location bookmarks
and a starting camera position.

<Warning>Context is beta. Avoid relying on it for production-critical dashboards.</Warning>

Map tiles require an internet connection, and OpenFreeMap attribution stays visible. Deleting a
Map or Context widget does not clear the dashboard site location.
