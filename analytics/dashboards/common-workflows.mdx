...
<Steps>
  ...
  <Step title="Add Total property value or Count by property for breakdowns">
    Add **Total property value** for area or length KPIs, or **Count by property** for counts by
...
<Steps>
  ...
  <Step title="Use the view link">
    Open the dashboard in view mode (not edit). Share that URL so recipients see the dashboard
    without edit controls. See [Layout and sharing](/analytics/dashboards/layout-and-sharing) for
    view-only and sharing options.
  </Step>
...
<Info>
  The **Context** widget is different from [context
  models](/analytics/intelligence-dashboards#context-models) in the Model Viewer (site geometry that
  stays visible while filters isolate design schemes).
</Info>
...
<Steps>
  ...
  <Step title="Set the dashboard site location">
    Open the Map widget sidebar **Site** section. Enter latitude and longitude, then **Apply**, or
    set the site from the map centre. Optional elevation and rotation update live with the sidebar
    sliders. The site location is owned by the dashboard — you do not need geographic data on the
    model. You can also set or clear it from the **Context** widget sidebar later.
  </Step>
  <Step title="Optionally add location pins">
    Use the sidebar **Locations** section to add the current map view as a bookmark, then fly to,
    rename, or remove pins as needed.
  </Step>
  <Step title="Add a Model Viewer and load your model">
    Drag **Model Viewer** onto the canvas and select your project and model. **Context** can reuse
    geometry from an existing viewer when one is already on the dashboard.
  </Step>
  <Step title="Add Context">
    Drag **Context** from **Presentation** onto the canvas. It places your model on a pitched 3D
    OpenFreeMap map at the dashboard site location. Set or clear the site in its sidebar **Site**
    section if you have not already done so on Map.
    {/* IMAGE: Context widget showing model on pitched 3D map */}
  </Step>
  <Step title="Optionally refine the Context view">
    Save a starting viewpoint with **Focus** (or the sidebar **Camera** section). Use **Clip model
    to ground** to hide geometry below the map deck. Use the bulldozer control to hide basemap
    buildings that overlap the model footprint, and set **Building clearance** in the sidebar if
    needed. Optionally fly the Map to the site from the Model Viewer toolbar with **View on map**.
  </Step>
</Steps>
...
<Tip>
  **Context** is marked **Beta** in the widget panel. Map tiles need an internet connection;
  OpenFreeMap attribution stays visible. Deleting Map widgets does not clear the dashboard site
  location.
</Tip>
## See also
