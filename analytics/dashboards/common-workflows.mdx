...
<Steps>
  ...
  <Step title="Set the dashboard site location">
    Click **Site** and enter latitude and longitude (optional elevation and rotation), or use **Set
    site from centre** to take the current map centre. The site location is owned by the dashboard —
    you do not need geographic data on the model.
  </Step>
  <Step title="Optionally add location pins">
    Search for a place, then click **Add location** to save bookmark pins on the map. Rename or
    remove pins from the location list as needed.
  </Step>
  <Step title="Add a Model Viewer and load your model">
    Drag **Model Viewer** onto the canvas and select your project and model. GeoSpeckle can reuse
    geometry from an existing viewer when one is already on the dashboard.
  </Step>
  <Step title="Add GeoSpeckle">
    Drag **GeoSpeckle** from **Presentation** (may appear as **Geo rendering (experiment)**) onto
    the canvas. It uses the site location you set on the Map widget and does not change it.
    {/* IMAGE: GeoSpeckle widget showing model on pitched 3D map */}
  </Step>
  <Step title="Optionally fly the Map to the site from the viewer">
    In the Model Viewer toolbar, use **View on map** to move the Map widget to the dashboard site
    location.
  </Step>
</Steps>
...
<Info>
  ...
  GeoSpeckle is the path for seeing the model on a map. The Model Viewer stays in local coordinates;
  GeoSpeckle handles the 3D-on-map view.
</Info>

<Tip>
  GeoSpeckle is **beta** and marked **Experimental** in the widget panel. Treat it as evolving — not
  for production-critical dashboards. Map tiles need an internet connection; OpenFreeMap attribution
  stays visible. Deleting Map widgets does not clear the dashboard site location.
</Tip>
## See also
