...
description: "Learn how to create validation rules and use rulesets in automations."
---
...
<Warning>
Image: Rule creation interface showing WHERE and CHECK conditions
</Warning>
...
<Warning>
Image: Rule with multiple AND conditions
</Warning>
...
Use exact property paths for accurate validation. For example:
- `baseline.length` instead of just `length` (if length exists in multiple locations)
- `properties.identity data.acmelimited_custompropertyA` for custom properties
...
<Warning>
Image: Copy link button on rulesets page
</Warning>
...
<Warning>
Image: Automate configuration showing Model Checker function and URL field
</Warning>
