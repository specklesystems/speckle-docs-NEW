...
- Every page under `next/` sets `contextual.options: []` and the incremental banner (`snippets/next-preview-banner.mdx`). Do **not** set `noindex: true` — 2026.9 is a public Mintlify version. Do not call 2026.9 a preview in reader-facing copy. The other Mintlify version stays **Current**: that is the complete corpus for topics without a 2026.9 page, not a marketing name.
- **"Current" is the version-selector label only, never a name in body copy.** A 2026.9 reader is on the current product, so "the Current model" reads as a contradiction. Write **before 2026.9** or **2026.8 and earlier**, or name the mechanism (the object graph, Proxies, the Root Collection). Table columns: `Before 2026.9` / `2026.9`, not `Today` / `Current`. Link the page by its title, not as "the Current Data Schema". The banner may say "switch to Current" because it names the selector. Do not use "Legacy" for the 2026.8 corpus either; that word names the v2 to v3 migration.
